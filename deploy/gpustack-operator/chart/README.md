...
| manufacturers | object | `{"amd":"1002","ascend":"19e5","cambricon":"cabc","hygon":"1d94","iluvatar":"1e3e","metax":"9999","mthreads":"1ed5","nvidia":"10de","thead":"1ded"}` | Accelerator manufacturers to manage, mapped to their PCI vendor ID. This is the single source for the worker `--manufacturer` list, the per-manufacturer device-manager DaemonSets, the GPUSTACK_<MANUFACTURER>_PCI_VENDOR_ID environment variable propagated to both the worker and the device-managers, and the vendor IDs the `gpustack-cpu-info` NodeFeatureRule detects. The manufacturer list is derived from these keys (sorted for stable rendering); each device-manager DaemonSet only schedules on nodes labelled by NFD with the matching PCI vendor, so adding an entry here is all it takes for a new vendor's devices to be labelled, detected and given a device-manager. The defaults are every manufacturer `pkg/nodefeature` knows, which a Go test asserts and the generated schema requires; a cluster that carries only some of them runs the other device-managers empty, since each one only schedules where its vendor label exists. |
...
| node-feature-discovery.worker.config.core.labelSources | list | `["cpu","pci","custom"]` | Feature sources the worker collects. `custom` is what makes the chart's `gpustack-cpu-info` NodeFeatureRule evaluate. |
