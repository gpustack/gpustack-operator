...
| deviceManager.createRuntimeClasses | bool | `true` | Create a RuntimeClass for every manufacturer whose `global.manufacturers` row sets `runtimeInjectsDriver`, where the cluster does not already have one. That is the only set whose container runtime is certain to be installed — see the note on `global.manufacturers`. Disable when something else provides them (e.g. a vendor GPU operator). |
