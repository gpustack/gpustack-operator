...
- **NVIDIA (HAMi-core).** `xbuild-nvidia-cuda-*` → `libvgpu.so`. Verifies artifacts/linking, single-card injection (`nvidia-smi` shows the sliced VRAM + the SM/compute limit), **multi-card per-device limits** on real GPUs, and that postInit's serialization **recovers when its holder is killed**.
...
- **Hygon DCU (nothing of ours).** Verification-only: the slice is a `vdev<N>.conf` the vendor's own DTK/hyhal runtime reads, so there is no builder stage, no artifact and no target argument — the cases verify without building. They cover the record shape the vendor parser accepts, the `vdev_id`/file-name rule that costs a container its accelerator when broken, memory-cap enforcement, the CU mask bounding real compute, and two slices on one card at once including the `pipe_id` collision. Offer it when `pkg/devicemanager/allocator/hygon/**` or Hygon's `device-manager preflight` path changes, which is a different trigger from the shim backends above.
...
```bash
git diff --name-only origin/main...HEAD | grep -E 'pack/(gpustack-operator/(Dockerfile|external/)|thead-ppu-devel/)|csrc/(thead|amd)/|pkg/devicemanager/allocator/hygon/'
```
...
- **Hardware cases require a real accelerator** (local or the ssh host): ASCEND-CASE 2/3/4 need an NPU; NVIDIA-CASE 2 needs a GPU, NVIDIA-CASE 3 needs **≥ 2** GPUs, NVIDIA-CASE 4's part C needs one (its parts A/B need none); THEAD-CASE 2/3/4 need a PPU, and THEAD-CASE 5 plus THEAD-CASE 6's Part C need **≥ 2 idle** ones; AMD-CASE 2..7 need an AMD GPU, and AMD-CASE 6's two-card row needs **≥ 2**; HYGON-CASE 2..5 need a DCU (one card is enough — case 5's two slices share it). The two CASE-1 builds need docker+buildx; THEAD-CASE 1 needs only a runtime that can `run`, AMD-CASE 1 needs neither when the target already carries ROCm, and HYGON-CASE 1 needs neither a card nor a runtime — only `/opt/hyhal`.
- **Hygon builds nothing, and that is deliberate** — its slice is a `vdev<N>.conf` the vendor's own DTK/hyhal runtime reads, so this repository ships no artifact for it (`csrc/` carries only `amd` and `thead`, and the image has no `xbuild-hygon` stage). Its cases therefore take no target argument and have no `build.sh` step: they verify without building. Do not add a backend for it — that would change what this skill is.
...
6. **Hygon** — no build step and no target argument; see the hard rule above.
   ```bash
   XB_MODE=… XB_HOST=… bash .../cases/hygon-case-1.sh    # no DCU; record shape + parser evidence
   XB_MODE=… XB_HOST=… XB_HCU=0 XB_MEM=1024 XB_CU=8 bash .../cases/hygon-case-2.sh
   … hygon-case-3.sh   # the memory cap is enforced, not just reported
   … hygon-case-4.sh   # the CU mask bounds compute (XB_WIDTHS, XB_RATIO)
   … hygon-case-5.sh   # two slices on one card + the pipe_id collision
   ```
   `XB_WORKLOAD_IMAGE` must carry torch for cases 2..5 (the reader that shows a slice must be a HIP
   client — `hy-smi` answers from the DMI layer and reports the physical card).
...
| 4 | postinit lock recovery after owner death | parts A/B no, part C yes (1 GPU) | postInit's host-PID detection used to be serialized by an **unnamed semaphore inside the shared region**, which nothing releases when its holder dies: the next process waited 30s×10 = **300s** and then ran without host-PID detection. It is now a **POSIX record lock** on a byte of the shared cache **file** at the fixed offset `0x40000000`, which the kernel releases on exit, `SIGKILL` included. **Part A** is the revert tripwire on the pinned commit: the shipped `libvgpu.so` must carry the record-lock strings and must **not** carry the semaphore give-up ones, asserted in both directions because a pin that silently moved back would otherwise show up only as a 300s stall on someone's node. **Part B** runs upstream's own GPU-free regression test built from the pinned source already staged in the image — it links `-lrt -lpthread` and no CUDA/NVML, kills a holder *and* the first waiter after it, and requires the second waiter to stay blocked while either is alive and to acquire promptly after each `SIGKILL`. **Part C** is the black box: the real injected library in a real container, killed **while the host can see it holding the lock**. Each round starts a holder, spins on `/proc/locks` until a POSIX WRITE entry appears on the shared cache file's inode at that offset, SIGKILLs the holder at that moment, and requires the next container to finish inside `XB_RECOVER_TIMEOUT` (60s, a bound the old 300s give-up blows) logging **neither** give-up line; afterwards no entry for that inode may survive. The kill is triggered by the lock, never by a delay: a first version fired at staggered offsets across the postInit window and passed **5/5 against the old pin too**, because no kill had landed inside the window — five green rows attesting to nothing. A round that cannot observe the lock before it fires is now reported **unarmed** and counted as evidence of nothing, and with the record-lock path shipped a run that arms nothing FAILs. The mechanism row is decided by what part A found: absent-and-record-lock-shipped is a defect, absent-and-semaphore-shipped is the correct answer, which is what lets this case be pointed at an older pin as a control |
...
### Hygon DCU (no builder stage and no artifact of ours; the vendor's DTK/hyhal reads a `vdev<N>.conf`)
| Case | Title | Needs DCU | Asserts |
|---|---|---|---|
| 1 | Record shape + parser evidence | no | the eight fields `vdevConf.render` emits, in order, every line `key: value`, `mem` carrying a MiB unit, and the **two consistency rules the file itself does not state**: `cu_count` == popcount(`cu_mask`), and `vdev_id` == the ordinal in its own file name. Then the vendor parser's own strings out of `libhsa-runtime64.so` — all eight field names, `/etc/vdev/docker/`, `%s/vdev%u.conf`, and both diagnostics — so the rules have a second reader that is not ours. Needs `/opt/hyhal`, no card and no container runtime |
| 2 | Inject one slice | yes | `total_memory` and `multi_processor_count` fall from the whole card to the record's `mem`/`cu_count`, and the quota is **below** the card (equal figures would pass on a card the size of the quota). Then the `vdev_id` rule as a **negative**: the same record named `vdev0.conf` while carrying `vdev_id 1` leaves the container with **no accelerator at all** — not a smaller slice — and named `vdev1.conf` it works. Finally the reader `device-manager preflight` itself uses: `BandwidthTest` out of the mounted DTK tree makes the driver publish the vgpu instance and the record reports the quota in MiB, while `hy-smi` under the same injection reports the **physical** card, which is why it is not that reader |
| 3 | Memory-cap enforcement | yes | half the quota allocates; twice and eight times it fail — both **far inside** the physical card, so a full card or a blanket refusal cannot explain the pattern — and HIP names the quota back as the capacity |
| 4 | CU mask bounds compute | yes | the same matmul under several mask widths: each width's reported CU count equals the mask's weight, throughput rises from the **narrowest to the widest** (the ends, not adjacent pairs, which invert on a shared card's jitter), the narrow one is under a stated fraction of the wide one, and a **full** mask matches the unsliced card — the row that separates isolation from the record costing performance |
| 5 | Two slices on one card | yes | two containers behind a shared-directory barrier so both are alive when either reads, holding **different** quotas from one card, each allocating inside its own. Both records carry `vdev_id 0` — legal across containers, and required, since each is its own pod's `vdev0.conf` — and the driver lists an instance per slice carrying `Vgpu device:` and each `Vram limit`, which is what preflight reads. The negative half: two live slices sharing one `pipe_id` costs the second its device |
...
- NVIDIA: `XB_GPU` (single-card index, 0); `XB_GPUS` (multi-card csv, `0,1`); `XB_MEM` (MiB, 4096);
  `XB_SM` (compute %, 50 / 30); `XB_KILL_ROUNDS` (case 4, 3) and `XB_ARM_TIMEOUT` (case 4, 30s — how
  long a round spins on `/proc/locks` for the holder's lock before firing unarmed) and
  `XB_RECOVER_TIMEOUT` (case 4, 60s — must stay well under the old 300s give-up for the recovery rows to
...
- Hygon: `XB_HCU` (the `hy-smi` card index, 0 — each case resolves its PCI bus id and both drm minors
  from it, so nothing is keyed on a drm number); `XB_MEM` (the record's `mem`, 1024 MiB; case 4 uses
  3072 since it holds three 4096² fp16 matrices); `XB_CU` (the mask's weight, 8); `XB_MEM_A`/`XB_MEM_B`
  (case 5's two quotas, 1024 / 2048 — they must **differ**, or one shared slice would satisfy both
  rows); `XB_WIDTHS` (case 4's mask widths, `8 20 40`; the card's full width is appended automatically)
  and `XB_RATIO` (case 4, 50 — the narrowest must come in under this percentage of the widest);
  `XB_HYHAL` (`/opt/hyhal`) and `XB_DTK` (`/opt/dtk`, mounted at `/opt/hygondriver`); `XB_BDF` (case 1
  only — any well-formed bus id, since it opens no card). There is no `XB_STAGE`: nothing is staged,
  because this repository builds nothing for Hygon.
