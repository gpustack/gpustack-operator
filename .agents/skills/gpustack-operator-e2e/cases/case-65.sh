#!/usr/bin/env bash
#
# CASE 65 — A member group's local disk tier takes real bytes: evicted objects land in the
#   host directory and stay readable, on the one write path the API offers   (MUTATING,
#   self-recovering; AUTO-SKIPS without a prepared host directory on every node)
#
#   case-65.sh <NS>
#
# Goal:        A group declaring localDisks renders a tier the leader accepts and publishes, but a
#              published capacity is not a written byte — the store has been observed announcing
#              the tier while nothing ever reached the disk. The tier has ONE write path now, and
#              the API removed the other on purpose: a key's bytes move to disk at eviction time,
#              never at put time, because the write-at-put mode left an unflushed bucket's sole
#              replica one eviction away from destruction. So this case forces eviction — it
#              writes one member's capacity beyond the aggregate memory of the members actually
#              running — and then asks three things only real bytes answer: the leader's own
#              master_allocated_file_size_bytes reads above zero, files exist in the host
#              directory, and a key the leader reports as holding a disk replica but no memory one
#              reads back the exact bytes written under it. The figure follows a bucket being
#              closed rather than a put returning, so it is asked for repeatedly until it carries
#              a number; and before any of it is read as a verdict, the case reads the rendered
#              bucket limit out of the member container and asserts its write set clears one
#              bucket PER MEMBER, since each member client fills its own while the gauge sums the
#              backend.
#
...
#
# Inputs:      One KVCacheBackend whose single member group declares localDisks{path,
#              capacity}. The offload flags are derived from that declaration, in the deferred
#              mode, and nothing about the tier is stated anywhere else on the object. Memory
#              per member is sized small (256Mi) so eviction pressure is cheap to induce. Probe
#              Pods (the store image's bundled python client, P2P handshake against the leader
#              Service) write 4MiB objects whose content is derived from each key's own name.
...
#                clears it — without this the next assertions could not be read as verdicts;
#              - writes succeed, INCLUDING the ones the store first refuses for want of memory:
#                that refusal is the moment the master flags the eviction this case exists to
#                force, so the put is retried rather than taken for a stop;
...
#                reads back byte-identical against a payload derived from its own key;
...
#              defaults a bucket to 256 MB, the old write set offered less than that per member,
#              and below one bucket a tier legitimately holds nothing. Once the operator began
#              rendering the bucket limit the same write set closes eleven of them. Measured
#              green end to end on one member of 256Mi with a rendered 16 MiB bucket. So a FAIL
...
# transport alone, and a check that takes such a failure for an answer reports a verdict about
# the network rather than about the operator.
...
# No offload block on the leader, and that absence is the contract under test: declaring a tier is
# what turns the tier on, the mode is deferred by construction, and an object whose only replica
# survives is one that left memory by eviction rather than at put time.
...
if [ "$MEMBER_COUNT" -le 0 ] 2>/dev/null; then
  record FAIL "running members exist to force eviction" "no Running member Pod belongs to ${BACKEND}"
  results; exit 1
fi
...
WRITE_TOTAL=$((MEMBER_COUNT * 64 + 64))
...
BYTES_PER_MEMBER=$(((WRITE_TOTAL * 4 * 1024 * 1024) / MEMBER_COUNT))
...
if [ "$BYTES_PER_MEMBER" -lt "$BUCKET_LIMIT" ] 2>/dev/null; then
  record FAIL "the write set clears one bucket per member" \
    "the case offers ${BYTES_PER_MEMBER} bytes per member across ${MEMBER_COUNT} member(s) \
...
# ------------------------------------------------- 2. writes past aggregate memory, then the placement answer
...
# from a previous run's leftovers produces -- compares equal and passes.
def payload(key):
    seed = (key + "|case65-onevict|").encode()
    return (seed * (OBJ // len(seed) + 1))[:OBJ]

def query(keys):
    url = "http://${ADMIN_ADDR}/batch_query_keys?keys=" + ",".join(keys)
    raw = urllib.request.urlopen(url, timeout=60).read().decode()
    return (json.loads(raw).get("data") or {})

KEYS = ["oe-%03d" % i for i in range(${WRITE_TOTAL})]

# A put returning -200 (NO_AVAILABLE_HANDLE) IS NOT A TERMINAL ERROR HERE, and treating it as one
# is how this case came to assert a scene that never happened. The master raises its
# need-eviction flag on the very path that returns that code, and its eviction thread then runs
# the eviction -- which, on the tier's one write path, is what writes to the disk. So the first
# -200 arrives at the exact moment the thing this case exists to force is about to start, and
# stopping there stops just short of it. Retrying is what lets it happen.
#
...
for attempt in range(12):
    try:
        ...
        # case acts on, so a failed query that returned it would manufacture the verdict.
...
FILL="$(grep -E '^FILL written=' "$PROBE_LOG" | head -1)"
...
if [ -z "$FILL_W" ] || [ -z "$FILL_N" ]; then
  record FAIL "writes succeed past aggregate memory" \
    "no FILL line from the probe: $(grep -E 'SETUP|IMPORT-FAIL' "$PROBE_LOG" | head -2 | tr '\n' ' ')"
  ...
  record PASS "writes succeed past aggregate memory" \
    "${FILL} for ${MEMBER_COUNT} running member(s) (eviction forced)"
else
  record FAIL "writes succeed past aggregate memory" \
    "${FILL} $(grep '^FILL-HARD' "$PROBE_LOG") -- puts that no retry recovered, so the set that \
...
# right bytes is compatible with the object never having left memory, and this case's whole claim
# is that it did -- so the leader's own placement answer, taken before any read, is what makes
# the next assertion a statement about the TIER rather than about the store in general.
PLACE="$(grep -E '^PLACEMENT try=' "$PROBE_LOG" | tail -1)"
PLACE_T="$(echo "$PLACE" | sed -n 's/.*tier_only=\([0-9][0-9]*\).*/\1/p')"
if [ -z "$PLACE_T" ]; then
  # An unanswered query is NOT "nothing is on the tier". That reading is the one this case acts
  ...
  record FAIL "the leader holds keys on the tier that memory no longer has" \
    "no placement reading: ${PLACE:-<no PLACEMENT line>} $(grep '^PLACEMENT-FAIL' "$PROBE_LOG") \
...
SUBJECT="$(grep -E '^SUBJECT ' "$PROBE_LOG" | head -1 | awk '{print $2}')"
GET_SHA="$(grep -E '^GET ' "$PROBE_LOG" | head -1 | awk '{print $4}')"
WANT_SHA="$(grep -E '^WANT ' "$PROBE_LOG" | head -1 | awk '{print $4}')"
if [ -n "$WANT_SHA" ] && [ "$GET_SHA" = "$WANT_SHA" ]; then
  record PASS "an evicted object reads back byte-identical from the tier" \
    "$(grep -E '^GET ' "$PROBE_LOG" | head -1) [${PLACE}]"
else
  ...
  record FAIL "an evicted object reads back byte-identical from the tier" \
    "subject=${SUBJECT:-<none: no key was on the tier>} get: $(grep -E '^GET ' "$PROBE_LOG" | head -1) \
vs $(grep -E '^WANT ' "$PROBE_LOG" | head -1) -- NotFound means eviction dropped the object instead \
