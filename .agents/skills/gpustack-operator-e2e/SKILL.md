...
| 65 | A member group's local disk tier takes real bytes on BOTH write paths the store offers: write-through (offload enabled) and eviction-time (`offload.onEvict=true`, with writes pushed past aggregate memory so eviction is forced) — the leader's `master_allocated_file_size_bytes`, not the declared `status.capacity`, is the verdict figure, and phase B's outcome says which path a fix has to reach | `pkg/worker/kvcache/mooncake/member_workload.go` (the localDisk env render), `pkg/worker/kvcache/mooncake/leader_workload.go` (the offload flags), `api/worker/v1alpha1` (`members[].localDisk`, `leader.offload`) | yes (confirm) | as CASE 62 (same image need) + `E2E_LOCALDISK_HOST_PATH` pointing at a prepared writable directory on EVERY Ready node (e.g. an i7ie's NVMe at `/mnt/kvcache-localdisk`), else AUTO-SKIP. NO LONGER a known-failure detector: the metric and host-directory checks were expected to FAIL while the store ran on its own 256 MB bucket default, which phase A's write set never closed; with the bucket limit rendered they pass, so a FAIL is now a regression to act on |
