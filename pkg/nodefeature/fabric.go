...
func ConstructFabricNodeLabels(groups device.DevicesGroupList) map[string]string {
	...
	// The domain value is composite, so the sanitizer is not a safety net for it: it caps a label
...
// domain established and its size unknown. Taking whichever count the iteration reached first would
// let two nodes in one super pod publish different sizes for it with nothing recording that the
// inputs disagreed, so the count is withheld instead. Zero is how ConstructFabricNodeLabels already
// spells a size nobody reported.
