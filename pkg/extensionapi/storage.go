...
type (
	...
	// ReadOnlyOperationsHandler is an interface for read-only handler.
	ReadOnlyOperationsHandler interface {
		ListWatchHandler
		GetHandler
	}

	// ReadOnlyOperations implements the rest.Lister, rest.Watcher and rest.Getter interfaces.
	ReadOnlyOperations struct {
		ListWatchOperation
		GetOperation
	}

	// NoUpdateOperationsHandler is an interface for CURD handler without update operation.
	NoUpdateOperationsHandler interface {
		CreateHandler
		ListWatchHandler
		GetHandler
		DeleteHandler
	...
	// NoUpdateOperations implements the rest.Creater, rest.Lister, rest.Watcher, rest.Getter,
	// rest.GracefulDeleter and rest.CollectionDeleter interfaces, but without update operation.
	NoUpdateOperations struct {
		CreateOperation
		ListWatchOperation
		GetOperation
		DeleteOperation
...
var (
	...
	_ rest.Patcher           = (*CurdOperations)(nil)
	_ rest.GracefulDeleter   = (*CurdOperations)(nil)
	_ rest.CollectionDeleter = (*CurdOperations)(nil)

	_ rest.Lister  = (*ReadOnlyOperations)(nil)
	_ rest.Watcher = (*ReadOnlyOperations)(nil)
	_ rest.Getter  = (*ReadOnlyOperations)(nil)
)
...
// WithReadyOnly returns a ReadOnlyOperations for the given rest.TableConvertor and ReadOnlyOperationsHandler,
// which implements the rest.Lister, rest.Watcher and rest.Getter interfaces.
func WithReadyOnly(tc rest.TableConvertor, h ReadOnlyOperationsHandler) ReadOnlyOperations {
	l := WithListWatch(tc, h)
	g := WithGet(h)
...
// WithNoUpdate returns a NoUpdateOperations for the given rest.TableConvertor and NoUpdateOperationsHandler,
// which implements the rest.Creater, rest.Lister, rest.Watcher, rest.Getter, rest.GracefulDeleter and rest.CollectionDeleter interfaces,
// but without update operation.
func WithNoUpdate(tc rest.TableConvertor, h NoUpdateOperationsHandler) NoUpdateOperations {
	c := WithCreate(h)
	l := WithListWatch(tc, h)
	g := WithGet(h)
	d := WithDelete(h)
	return NoUpdateOperations{
		CreateOperation:           c,
		ListWatchOperation:        l,
		GetOperation:              g,
		DeleteOperation:           d,
