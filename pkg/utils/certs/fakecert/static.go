...
// StaticManager manages the self-signed host certificates,
// which generates the server certificate according to the given Host, AlternateIPs, and AlternateDNSNames names.
...
func (m *StaticManager) GetCertificate(hello *tls.ClientHelloInfo) (*tls.Certificate, error) {
	...
	if host != "" {
		...
		if addr == nil {
			return nil, errors.New("fakecert: missing host")
		}
		tcpAddr, ok := addr.(*net.TCPAddr)
		if !ok {
			return nil, errors.New("fakecert: invalid host")
		}
	...
	if !certs.RequestECDSA(hello) {
		return nil, errors.New("fakecert: disallowed ECDSA")
	}
