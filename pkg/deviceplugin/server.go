...
// which is what lets the node-wide allocate mutex and the per-accelerator cross-mode invariant
// reach across the modes rather than only within one. Responder carries the vendor-specific half of
// a response; Allocate refuses the request outright when it is unset.
type ResourceServer struct {
	...
	// returned while a generation was still being set up would leave that generation free to take a
	// socket path the next allocator has already claimed.
	done       chan struct{}
	server     *grpc.Server
	registered bool
}
...
func (s *ResourceServer) ListAndWatch(_ *Empty, srv grpc.ServerStreamingServer[ListAndWatchResponse]) error {
	// Get notifier at the beginning of ListAndWatch to avoid missing any update during the initial
	// ListAndWatch, and give it up on the way out: kubelet opens a fresh stream on every
	// registration, so a subscription that outlived its stream would accumulate one per kubelet
	// restart for the life of the process.
	notifier, release := s.Reconciler.getReconcileNotifier(s.Manufacturer, s.AllocationMode)
	defer release()
...
func (s *ResourceServer) getListAndWatchResponse(ctx context.Context) (*ListAndWatchResponse, error) {
	...
	// per-accelerator verdict, so it is published on its own.
...
// advertiseCard returns the tokens one accelerator contributes to this family's ListAndWatch
// response, or nothing at all when the accelerator lies outside the family's scope.
//
// This server's family draws tokens only from the accelerator population that can physically serve
// it, and sizes its pool from that accelerator's own capability: logical slicing and hardware
// partitioning are exclusive accelerator states, and a partitioned accelerator is no longer
// available as a whole accelerator. Scope, not health, is the mechanism — the populations are
// physically exclusive, so an accelerator skipped this way could never become servable while it
// stays in that state. Visibility is exempt and advertised everywhere: a visibility request must
// co-allocate the very accelerator its owner holds, whatever state that accelerator is in.
func (s *ResourceServer) advertiseCard(
	...
	case workercore.DeviceAllocationModeExclusive, workercore.DeviceAllocationModeShared:
		if !device.IsWholeAcceleratorCapable(acc.Status) {
			return nil
		}
	...
	// Hardware health alone does not protect an accelerator held in another allocation mode:
	...
	// NO TOPOLOGY IS AN ANSWER HERE, AND IT IS THE ANSWER WHEN THE KERNEL REPORTED NO AFFINITY.
	// An empty NumaAffinity means the accelerator's NUMA node is unknown -- the kernel's -1 in
	// /sys/bus/pci/devices/<bdf>/numa_node, which a virtualized host reports for every device -- so
	// no node is named and kubelet's topology manager is left unconstrained.
	//
	// The alternative was in place until it was measured: unknown used to arrive as "0", so every
	// accelerator on such a host advertised a hint of node 0. That is worse than saying nothing,
	// and the reason is asymmetric. On a single-NUMA host the two are equivalent. On a multi-socket
	// host whose driver does not report affinity, the hint pinned the device to a node nobody had
	// measured, and under the single-numa-node policy that hint is what decides admission: kubelet
	// would place the workload against a proximity claim the kernel had declined to make, the
	// placement would succeed, and only the performance would be wrong. A wrong hint became no
	// hint deliberately.
...
// node-level count — Allocate chooses the accelerator — so health answers "how many more instances
// can this node host", never "which accelerator is free". That also means a partition token carries
// no NUMA hint: it names no accelerator, so a hint would tell the TopologyManager something the
// token cannot honor.
//
// Every partitioned accelerator advertises its full ceiling of IDs, always. IDs are never removed:
// removing one strands the kubelet's checkpointed allocation for whatever container held it. What
// varies is how many are Healthy — summed over the node's partitioned accelerators, allocated +
// remaining, where remaining is the largest number of further instances an accelerator can still
// host over its profiles. The allocated term is required: the kubelet publishes the healthy count
// as allocatable and the scheduler then subtracts the requests of the Pods already on the node, so
// a bare remaining would lose one slot per live instance. A node with no room left therefore
// advertises exactly its live instance count, which the scheduler reduces to a free view of zero.
//
// The healthy SET is part of the contract, not just its size. The kubelet checkpoints the exact IDs
// it offered a container and, on any later allocation for it, refuses to proceed unless every one
// is still healthy — the check runs before the "no new devices needed" shortcut, and
// sanitizeNodeAllocatable does not rescue it. So every ID a live allocation holds stays Healthy for
// that allocation's life, and the free room is granted as a stable prefix of the rest: two cycles
// over an unchanged ledger publish the identical set.
func (s *ResourceServer) getPartitionListAndWatchResponse(
	...
	for i := range devs.Spec.Groups {
		...
		for j := range grp.Accelerators {
			...
			if acc.Status.Unhealthy {
				// A broken accelerator keeps its IDs — a live allocation may still hold one — but
...
// partitionRoomOf returns how many instances an accelerator currently accounts for: the ones it
// already carries plus the largest number of further ones it can still host over its profiles, per
// the placement-aware ledger. An accelerator whose ledger has not been published yet — a fresh
// node, or a device manager still rolling out — falls back to its static ceiling, so the node
// advertises its full room rather than none.
...
func (s *ResourceServer) GetPreferredAllocation(ctx context.Context, req *PreferredAllocationRequest) (*PreferredAllocationResponse, error) {
	// The visibility and partition token pools are flat and interchangeable — a visibility token
	// resolves to the pod's already-reserved device, and a partition token to whatever accelerator
	// Allocate chooses — so neither has a preference to express. Return an empty response (kubelet
	// picks freely) rather than run the sliced per-accelerator bin-fit, which assumes tokens map to
	// the real devices they name.
...
func (s *ResourceServer) getContainerPreferredAllocationResponse(
	...
	mustIncludedResTokensMap, err := parseResourceTokensByAccelerator(ctrReq.GetMustIncludeDeviceIDs())
	if err != nil {
	...
	if preferredDeviceIDsSet.Len() > 0 {
		...
		// remainingSize can be negative: the walk keeps taking preferred accelerators after the
		// claim is full, because it only stops early once the preferred set is ALSO empty — and a
		// "accelerator.preferred-id" annotation is taken at face value, so an ID naming no
		// accelerator on this node never clears from that set. A pod rescheduled onto a different
		// node carries exactly such an annotation. Without the lower bound the slice below panics
		// on a negative index and takes the device-manager down with it.
		if remainingSize > 0 && remainingSize <= int32(len(sel.Unselected)) {
			...
			selectedResTokens = append(selectedResTokens, sel.Unselected[:remainingSize]...)
			remainingSize = 0
		}
	}
	...
	deviceIDs := make([]string, 0, len(selectedResTokens))
	for i := range selectedResTokens {
		deviceIDs = append(deviceIDs, selectedResTokens[i].String())
	}
...
// parseResourceTokensByAccelerator parses device IDs into their ResourceTokens, grouped by the
// accelerator each one names. The IDs are sorted first, so an accelerator's tokens land in a stable
// order and a caller taking the first of them gets the same token on every call.
func parseResourceTokensByAccelerator(deviceIDs []string) (map[Resource][]ResourceToken, error) {
	sort.Strings(deviceIDs)

	byAccelerator := make(map[Resource][]ResourceToken)
	for i := range deviceIDs {
		resToken, err := ParseResourceToken(deviceIDs[i])
		if err != nil {
			return nil, fmt.Errorf("%q: %w", deviceIDs[i], err)
		}
		byAccelerator[resToken.Resource] = append(byAccelerator[resToken.Resource], resToken)
	}

	return byAccelerator, nil
}

// _PreferredSelection is what one walk of this node's accelerators produced for an allocation hint.
type _PreferredSelection struct {
	// Selected are the tokens the hint will offer, at most one per accelerator.
	Selected []ResourceToken
	// Unselected are the tokens of accelerators passed over: one that cannot fit this slice's
	// per-accelerator units, or one outside the preferred set. Only the preferred-accelerator
	// fallback draws on them.
	Unselected []ResourceToken
	// RemainingSize is how much of the requested allocation the walk could not satisfy.
	RemainingSize int32
}

// selectPreferredUnits walks this manufacturer's accelerators in packing order and takes one token
// per accelerator until the claim is full and every preferred accelerator has been honored, which
// is the point it returns from. Returning is what lets it leave both loops at once; the two used to
// need a goto.
//
// preferredDeviceIDsSet is narrowed in place as accelerators are honored, so what remains in it
// after the walk is exactly the preferred accelerators that could not be met — which is how the
...
func (s *ResourceServer) selectPreferredUnits(
	...
	for i := range devs.Spec.Groups {
		...
		for _, j := range slicedPackingOrder(devs, grp, mustIncludedResTokensMap, slicedUnits) {
			...
			resTokens, existed := availableResTokensMap[res]
			if !existed {
				continue
			}
			...
			// Exclusive, shared and sliced all select one device token per accelerator; the
			// per-accelerator concurrency/units accounting lives elsewhere (Kueue credits and the
			// ".sliced.units" capacity), not in the device plugin.
			if mustUnits, existed := mustIncludedResTokensMap[res]; existed {
				// Only the first must-include token per accelerator is consumed. The
				...
				// win the accelerator back. Echoing it still matters: it spends one of the claim's
				// slots, which is what leaves that intersection holding exactly the devices kubelet
				// still needs rather than a wider set it would then pick from arbitrarily. This is
				// why slicedPackingOrder visits these accelerators first.
				//
				// It is echoed whatever the ledger reports free — the fit check below deliberately
				// sits on the other branch. This token's units are ALREADY counted in that
				// accelerator's Remaining, so measuring it against Remaining charges the container
				// for its own claim a second time and would drop a token the response has to carry.
				...
				preferredDeviceIDsSet.Delete(res.Device)
				sel.Selected = append(sel.Selected, resTokens[0])
			}
			sel.RemainingSize--
...
// slicedUnits should try them. A non-positive slicedUnits leaves the accelerators in their natural
// order.
//
// An accelerator kubelet already allocated to this container (mustInclude) comes first whatever its
...
func (s *ResourceServer) Allocate(ctx context.Context, req *AllocateRequest) (*AllocateResponse, error) {
	...
	allocatedResTokensMap := make(map[Resource][]ResourceToken)
	for i := range allocatedDeviceIDs {
		resToken, err := ParseResourceToken(allocatedDeviceIDs[i])
		if err != nil {
	...
	// resolve to DISTINCT pods — getAllocatingPod skips pods a prior Allocate already reserved, and
	// that reservation is written there before the next Allocate reads it — and stops an
	// opposite-mode Allocate for the same accelerator from interleaving between the check and the
	// reservation (TOCTOU).
	//
	// The lock is taken here rather than inside decideAllocation so the serialized region is one
	// named call at the point it is entered, and defer releases it on every path including a panic:
	// a leaked allocate mutex would hang every later Allocate on this node.
...
type _AllocationDecision struct {
	...
	// Allocation is the per-accelerator units charge the responders are handed.
...
func (s *ResourceServer) decideAllocation(
	...
	// serialized Allocate places against an accelerator whose window is already spoken for.
	// Choosing it later — in the responder, after the mutex — cannot be made race-free.
	if placesLogical {
	...
	// Publish the partition selection — accelerator, profile and the intervals it intends to occupy
	// — before the reservation for the same reason, so the next serialized Allocate decides against
	// an accelerator already spoken for rather than one that merely has not been carved yet.
	...
	// and getAllocatingPod's skip-reserved), and a visibility Allocate can co-allocate the same
	// physical device without racing the annotation's cache propagation. The offered device IDs
	// ride along so ListAndWatch can keep advertising exactly them Healthy for as long as this
	// allocation lives.
...
// against: the Devices ledger, plus the node's live per-accelerator occupancy for the families that
// choose a position on the accelerator themselves. Reading it once keeps the candidate test and the
// placement decision on the same snapshot. Every read is served from the informer cache, which is
// what makes it legal under the allocate mutex.
func (s *ResourceServer) snapshotForAllocate(ctx context.Context, placesLogical bool) (
	*workercore.Devices, Placements, Placements, error,
) {
	devs, err := s.Reconciler.getDevices(ctx)
	if err != nil {
	...
	// per-accelerator occupancy: the annotations every live allocation carries, unioned with the
	// selections in-flight allocations have already published.
	...
	// A logical slice that occupies a position on the accelerator needs the same snapshot in its
	// own key space: what every live allocation already occupies, so this one can be placed beside
	// them rather than on top of them.
	var occupiedLogical Placements
	if placesLogical {
...
// a placement decision must read — the annotation source alone lags by a cache round-trip, and the
// reservation alone forgets everything across a restart.
...
func (s *ResourceServer) occupiedLogicalPlacements(ctx context.Context) (Placements, error) {
...
// candidateFeasible returns the test that tells two otherwise identical Allocate candidates apart.
// The RPC carries only a resource name and a device-ID count, so two pending containers asking for
// the same family are indistinguishable to it even when they demand very different things — and
// picking the wrong one actuates the wrong request. This narrows the choice using the one thing the
// plugin can check: whether the candidate's demand can still be served by the node, per the ledger.
//
// A logical slice's demand is its per-accelerator ".sliced.units", the normalized budget the Pod
// webhook folded its memory request into, tested against the accelerators kubelet offered. A
// partition's demand is its profile geometry, tested against the whole node — the offered tokens
// name no accelerator the allocation will use, so only the node's free placements can rule a
// candidate out. The exclusive and shared families carry no such dimension — every candidate for
// them really is interchangeable — so they get no test.
...
// partitionCandidates builds the placement decision's input from the live state: every partitioned
// accelerator of this server's manufacturer that offers the profile, with the profile's legal
// placements from the accelerator's detect-time capability and the intervals already taken. The
// second return maps a candidate ID back to its resource, since the selector deals only in opaque
// IDs. An accelerator offering no placement for the profile is left out entirely, so the selector
...
// when the accelerator does not offer it (or has not published its placement ledger yet).
...
// requestedUnits reports the per-accelerator counting units one token commits — the normalized
// value the Pod webhook folds the memory budget into (".sliced.units" for a logical slice,
// ".partitioned.units" for a partition) — and whether the container requested it at all. Recording
// that real cost rather than the loose token count keeps the per-accelerator ledger honest, so the
// node-devices admission check refuses an accelerator whose committed units would exceed capacity
// and the InstanceType views report the true remaining. It falls back to one unit per token for a
// Pod the webhook did not shape; whether that fallback is usable is the caller's judgement, since
// for a partition one token is a whole accelerator.
...
// choosePartitionCards decides which accelerators a partition allocation lands on, and at what unit
// cost. This family reads kubelet's tokens as a quantity and picks the accelerators itself, against
// the live occupancy: kubelet cannot know which accelerator can host a given geometry, and a wrong
// pick is the one placement error the plugin can repair rather than reject — so a rejection here
// means the node has no room at all. It records the profile on d and returns the chosen
// accelerators, the intervals they intend to occupy, and the unit cost, which it may fold from the
// profile.
func (s *ResourceServer) choosePartitionCards(
	...
	unitsPerToken int64,
	unitsRequested bool,
) (map[Resource][]ResourceToken, Placements, int64, error) {
	...
	// A partition names its own cost even when nothing shaped the request. The Pod webhook is
	// scoped to queued Pods, so a partition Pod submitted outside the scheduling chain carries no
	// units key — and for this family the token count is not a usable stand-in: one token is one
	// whole accelerator, which would charge a single unit for a partition that occupies half the
	// accelerator or all of it, leaving an accelerator carved to capacity reading as untouched in
	// the scalar ledger. The profile is the missing budget, so fold it here the way the webhook
	// would have.
	if !unitsRequested {
	...
	// A retried Allocate must land back on the accelerator it already used. The kubelet re-runs
	// Allocate for a container whose checkpoint it lost — a restart while the container was stopped
	// — and by then this container's own placement is part of the node's occupancy. Deciding afresh
	// would read it as somebody else's: a whole-accelerator profile would report the node
	// exhausted, and a node with a free sibling would place on THAT accelerator, bypassing the
	// vendor's per-(pod, container, accelerator) reuse marker and carving a second instance. The
	// accelerator-bound families get this for free, since kubelet re-offers the tokens it
	// checkpointed.
	var (
		cardTokens map[Resource][]ResourceToken
...
// partitionProfileUnits folds a partition profile into the per-accelerator counting units one
// instance of it commits, from the capability the detector published: the profile's own
// per-instance memory as a share of one whole accelerator's. It is the same VRAM-anchored fold the
// Pod webhook performs, so a partition costs the same whether or not the webhook shaped the
// request.
//
// The profile is a property of the product, so the first accelerator offering it answers for the
// group. Zero means the group cannot answer — an unknown profile, or a capability published before
// its memory detail was — and the caller keeps whatever budget it already had rather than charging
// a figure it could not derive.
...
// priorAllocationOf returns the allocation this container already holds: the in-process reservation
// first, then the durable annotation, which is the only record that survives a device-manager
// restart. It is what makes a retried Allocate idempotent, for a partition's accelerator and
// interval and for a logical slice's window alike — the lookup is mode-agnostic, and each caller
// reads out the part of the record it owns.
func (s *ResourceServer) priorAllocationOf(
	...
	if err != nil {
		// The annotation is unreadable; say so loudly and decide afresh, which is the only thing
		// left to do — but the operator needs to know the accelerator may be double-carved.
...
func priorPartitionTokens(
...
) (map[Resource][]ResourceToken, Placements) {
	...
	for i := range prior.Groups {
		...
		for j := range grp.Accelerators {
			acc := &grp.Accelerators[j]
			res := Resource{Group: grp.ID, Device: acc.ID}
			tokens[res] = append(tokens[res], ResourceToken{Resource: res, Index: uint64(len(tokens[res]))})
			// A physical record is a profile together with the intervals its instance occupies;
			// an accelerator carrying only half of one records nothing here.
...
// partitionTokens turns the placement decision into the two per-accelerator maps the rest of
// Allocate works in: one token per chosen accelerator (the ledger cost is per token, and a
// partition request takes exactly one instance per accelerator), and the intervals each
// accelerator's instance intends to occupy.
func partitionTokens(
	selections []device.PartitionSelection, byID map[string]Resource,
) (map[Resource][]ResourceToken, Placements) {
	tokens := make(map[Resource][]ResourceToken, len(selections))
	placements := make(Placements, len(selections))
...
// rejectCrossModeCards enforces the per-accelerator cross-mode invariant at the authoritative
// on-node gate: it refuses an accelerator another mode already holds, per the ledger Status OR the
// in-process reservation, so an exclusive tenant truly owns its accelerator on every path, Kueue or
// raw. Free (None) and same-mode accelerators (e.g. sliced-on-sliced) pass. The partition selector
// already skipped held accelerators, so for that family this re-checks its own choice rather than
// kubelet's.
func (s *ResourceServer) rejectCrossModeCards(
	devs *workercore.Devices, cardTokens map[Resource][]ResourceToken, pod *core.Pod,
) error {
...
// commits, building the allocation status the reservation publishes and the annotation persists.
func (s *ResourceServer) accumulateAllocation(
	devs *workercore.Devices, cardTokens map[Resource][]ResourceToken, unitsPerToken int64,
	...
	for i := range devs.Spec.Groups {
		...
		for j := range specGrp.Accelerators {
			...
			resTokens, existed := cardTokens[res]
			if !existed {
				continue
			}
			var allocated int32
			switch s.AllocationMode {
			default:
				allocated = nodefeature.ResourceMaxUnits // a whole accelerator
			...
			case workercore.DeviceAllocationModeSliced, workercore.DeviceAllocationModePartitioned:
				// Real per-accelerator units this container commits on the accelerator, so the
				// ledger reflects capacity rather than the loose token count. A partition needs its
				// own branch here for the same reason a slice does, and for one more: the default
...
// placeLogicalSlice picks the geometry this container will occupy on each allocated accelerator,
// reusing the window it already holds when the kubelet is re-running an Allocate whose checkpoint
// it lost: by then the container's own window is part of the node's occupancy, so deciding afresh
// would read it as somebody else's, move the container to a different window, and strand the first
// one until the Pod goes away.
func (s *ResourceServer) placeLogicalSlice(
	ctx context.Context,
	d *_AllocationDecision,
	logicalPlacer LogicalSlicedResponder,
	occupiedLogical Placements,
) (Placements, error) {
...
func (s *ResourceServer) priorLogicalPlacements(pod *core.Pod, container string) (Placements, bool) {
...
// the reservation publishes it and the annotation patch persists it.
func applyLogicalPlacements(status *workercore.DevicesStatus, placements Placements) {
	for i := range status.Groups {
		grp := &status.Groups[i]
		for j := range grp.Accelerators {
			acc := &grp.Accelerators[j]
			if p, ok := placements[Resource{Group: grp.ID, Device: acc.ID}]; ok && len(p) > 0 {
				acc.AllocatedLogicalPlacements = p
			}
		}
	}
}
...
// a CU window recorded on an accelerator for a container kubelet never starts, and it creates
// nothing but a directory, so rendering it early costs nothing if the patch then fails.
//
...
// accelerator was free, and their reclaimers — which preserve an instance whose Pod is still live —
// would keep it that way. They stay below the patch, and the strand their own failure can leave is
...
// annotation absent, the Pod-delete watch (gated on it) would never enqueue a prune, so the
// accelerator would stay stranded for the opposite mode. Kubelet does not start the container on
// this error, so freeing the reservation now is safe and keeps the release counting honest. A
// partition materialized beforehand is torn down too, so no half-owned instance persists past a
// failed patch.
...
// visibility containers, and without a per-container claim every one of them resolves to the oldest
...
// visibilityResponse renders the visibility container's response over the accelerators its owner
// holds.
//
// A partition-backed owner is answered by the responder's partition capability, because a
// visibility allocation is a device-cgroup grant and nothing else: naming the parent accelerator
// would open every partition carved on it, including other tenants'. The trigger is the owner
// container's own resource request, which is in the Pod spec from the start and therefore immune to
// the order in which the workload's Allocate publishes and re-publishes its reservation.
//
...
// keeps the accelerator-based response unchanged.
...
func (s *ResourceServer) isRegistered() bool {
	s.mu.Lock()
	defer s.mu.Unlock()
...
// Start serves this server's resource to kubelet for as long as ctx allows, re-establishing the
// service whenever kubelet takes it away.
//
...
// could not be established at all. Calling it on a server that is already running starts nothing
...
func (s *ResourceServer) Start(ctx context.Context, kubeSocket string) error {
	...
	for {
		if err := s.serve(ctx, kubeSocket, socket); err != nil {
			return err
		}
		if ctx.Err() != nil {
		...
		s.Logger.Info("generation is over, serving again", "socket", socket)
	}
}

// serve runs one generation of service: it listens on this server's own socket, serves the
// device-plugin gRPC API there, and registers the resource with kubelet, retrying the registration
// for as long as the generation lives — right after kubelet wiped the plugin directory its
// registration server may not be listening yet, and giving up would leave the resource unknown to
// the node.
//
...
func (s *ResourceServer) serve(ctx context.Context, kubeSocket, socket string) error {
	...
	deviceplugin.RegisterDevicePluginServer(srv, s)
	s.publish(srv)
	defer s.retire(srv)
	...
	gox.Go(func() {
		...
		if err := srv.Serve(lis); err != nil && !errors.Is(err, grpc.ErrServerStopped) {
			s.Logger.Error(err, "serve on socket", "socket", socket)
		}
	})
	...
	for {
		...
		if !s.isRegistered() {
			...
			if err := s.register(ctx, srv, kubeSocket, socket); err != nil {
				s.Logger.Error(err, "registering to kubelet, retrying", "socket", kubeSocket)
			}
		}
		select {
		case <-ctx.Done():
		...
		case <-served:
			...
			if ctx.Err() == nil {
				s.Logger.Error(nil, "stopped serving with its socket still in place", "socket", socket)
			}
		...
		if _, err := os.Stat(socket); err != nil {
			s.Logger.Info("socket is gone", "socket", socket)
			return nil
		}
	}
}

// begin marks this server as running and records how to stop it, reporting false when it is already
// running — in which case the caller must not serve.
func (s *ResourceServer) begin(stop context.CancelFunc) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *ResourceServer) publish(srv *grpc.Server) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *ResourceServer) retire(srv *grpc.Server) {
	s.mu.Lock()
...
// Stop stops the gRPC server and ends the serving loop, and does not return until Start has: past it,
// no generation of this server is left to touch the socket, which is what lets the allocator replace
// the server on that path. It is a no-op, logged, on a server that was never started. In-flight
// Allocates are abandoned rather than drained: kubelet retries an allocation it did not get an answer
// for, and the reservations this server took are pruned by the reconciler once their Pods are gone.
//
...
func (s *ResourceServer) register(ctx context.Context, srv *grpc.Server, kubeSocket, socket string) error {
	...
	if _, err = regCli.Register(regCtx, regReq); err != nil {
		return fmt.Errorf("registering %s: %w", s.ResourceName(), err)
	}
...
// cardHeldInOtherMode reports whether the physical accelerator is currently held in a mode
// different from this server's, per the ledger Status OR the in-process reservations (the
// reservation is race-safe: written synchronously by every workload Allocate). Free (None) or
// same-mode is not a conflict. It returns the conflicting mode for logging.
...
// accelerator, or DeviceAllocationModeNone when the accelerator is free or absent from the Status.
...
// statusRemainingOf returns the units the Devices ledger still reports free on a physical
// accelerator. An accelerator absent from the Status is treated as untouched, i.e. a whole
// accelerator free.
