...
func (s *ResourceServer) ListAndWatch(_ *Empty, srv grpc.ServerStreamingServer[ListAndWatchResponse]) error {
	...
	err := waitx.RetryUntilSuccess(ctx, 2*time.Second, true, func(ctx context.Context) error {
...
func (s *ResourceServer) retire(srv *grpc.Server) {
	s.serving.retire(srv, s.Logger)
}

func (s *ResourceServer) register(ctx context.Context, srv *grpc.Server, kubeSocket, socket string) error {
	return s.serving.register(ctx, srv, kubeSocket, socket, s.servingOptions())
}

// servingOptions names what this server serves: the plugin answering the gRPC calls, which is this
// server itself; the resource it registers; and its socket beside kubelet's, by base name.
func (s *ResourceServer) servingOptions() servingOptions {
	return servingOptions{
		Plugin:       s,
		ResourceName: s.ResourceName(),
		SocketName:   stringx.Join(".", s.Manufacturer, strings.ToLower(s.AllocationMode.String()), "sock"),
	}
}
