...
// SlicedCoresPercent reads the per-accelerator compute (SM) percent a sliced container asks
// for from its ".sliced.cores-percentage" request, defaulting to 100 (a whole accelerator's
...
// SlicedMemoryMib derives the per-accelerator VRAM limit (MiB) a sliced container asks for,
// given the accelerator's total VRAM (acceleratorVRAMMib). ".sliced.memory-percentage" takes
// precedence — floor(pct/100 * acceleratorVRAMMib) — over the absolute ".sliced.memory-mib";
// the result is capped at the accelerator's VRAM. It errors when neither is set, so a sliced
// allocate fails loudly rather than silently exposing the whole accelerator's VRAM. Memory is
...
// consistent.
func SlicedMemoryMib(ctr *core.Container, memPctResName, memMibResName core.ResourceName, acceleratorVRAMMib int64) (int64, error) {
	if q, ok := ctr.Resources.Limits[memPctResName]; ok {
		if pct := q.Value(); pct > 0 {
			return min(acceleratorVRAMMib*pct/100, acceleratorVRAMMib), nil
		}
	}
	if q, ok := ctr.Resources.Limits[memMibResName]; ok {
		if mib := q.Value(); mib > 0 {
			return min(mib, acceleratorVRAMMib), nil
		}
	}
