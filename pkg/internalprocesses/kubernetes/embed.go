...
func (Embedded) Start(ctx context.Context) error {
	...
	if !osx.ExistsDevice("/dev/kmsg") {
		...
		return errors.New(`require "--privileged" flag to run gpustack container`)
	}

	// NB(thxCode): With cgroup v2,
	// in order to nest gpustack multiple times,
	...
	var (
		k3sDataDir       = system.SubDataDir("k3s")
	...
	if imgConfDir := system.SubConfDir("images"); osx.ExistsDir(imgConfDir) {
		...
		err := osx.Link(
			...
			osx.LinkInReplace())
		if err != nil {
			return fmt.Errorf("error link agent image: %w", err)
		}
	}
