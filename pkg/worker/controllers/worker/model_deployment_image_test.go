...
func TestSynthesizeModelDeploymentImage(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			why:     "admission refuses an empty version beside a role with no image, so reaching here means that rule moved",
		},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := SynthesizeModelDeploymentImage(tc.engine, tc.version, tc.detail)
			if tc.wantErr != "" {
