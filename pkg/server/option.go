...
func NewOptions() *Options {
	...
	opts.ManagerOptions.KubeLeaderElectionID = "server.gpustack.ai"
...
func (o *Options) Complete(ctx context.Context) (*Config, error) {
	...
	if o.CertDir == "" {
		...
		certCache, err := certcache.NewK8sCache(ctx,
			"server", lpCli.CoreV1().Secrets(kuberess.SystemNamespaceName))
		if err != nil {
	...
	// Peer.
	peerCpCfg := &peer.ControlPlaneConfig{
		BindAddress:            o.BindAddress,
		BindPort:               funcx.Ternary(o.BindPeerPort == 0, serve.BindPort+1, o.BindPeerPort),
		ClusterID:              getPeerClusterID(lpRestCfg),
		SelfIP:                 system.PrimaryIP.Get(),
	...
	return &Config{
		...
		PeerControlPlaneConfig: peerCpCfg,
	}, nil
}

// getPeerClusterID generates the peer cluster ID from the kubeconfig credential.
func getPeerClusterID(lpRestCfg *rest.Config) string {
	var raw []string
	switch {
	case lpRestCfg.Username != "" && lpRestCfg.Password != "":
		raw = []string{
			lpRestCfg.Username,
			lpRestCfg.Password,
		}
	case lpRestCfg.KeyData != nil && lpRestCfg.CertData != nil:
		raw = []string{
			string(lpRestCfg.KeyData),
			string(lpRestCfg.CertData),
		}
	case lpRestCfg.KeyFile != "" && lpRestCfg.CertFile != "":
		keyBytes, err := os.ReadFile(lpRestCfg.KeyFile)
		if err != nil {
			raw = append(raw, string(keyBytes))
		}
		certBytes, err := os.ReadFile(lpRestCfg.CertFile)
		if err != nil {
			raw = append(raw, string(certBytes))
		}
	case lpRestCfg.BearerToken != "":
		raw = []string{lpRestCfg.BearerToken}
	case lpRestCfg.BearerTokenFile != "":
		tokenBytes, err := os.ReadFile(lpRestCfg.BearerTokenFile)
		if err != nil {
			raw = []string{string(tokenBytes)}
		}
	}
	if len(raw) == 0 {
		klog.Warning("no credential found in kubeconfig, " +
			"using dummy peer token, " +
			"this may cause security risks, " +
			"please provide a kubeconfig with credential or set the peer token by yourself")
		raw = []string{"_DUMMY_PEER_TOKEN_"}
	}

	return stringx.SumByFNV64a(raw[0], raw[1:]...)
}
