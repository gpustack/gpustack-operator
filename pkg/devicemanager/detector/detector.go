...
func (d *Detector) reportDevices(ctx context.Context, eGroups device.DevicesGroupList) error {
	...
	case kerrors.IsNotFound(derr):
		// Not a failure, so not logged as one -- but not the whole node either. See
		// nodeWideReadSynced.
		logger.V(3).Info("the node stores no Devices yet, so this is its first pass",
			"node", ndName,
			"consequence", "the distance is reduced over this pass's accelerators only, "+
				"and the fabric labels wait for a pass that can see the whole node")
	default:
...
// nodeWideReadSynced says whether a Devices read showed this pass the whole node, which is what
// publishing -- or withholding -- a node-wide label needs.
//
// Only a stored record does, and NotFound is deliberately not one. It is the node's first pass
// rather than a failure, but "nothing is stored yet" is not the same fact as "this pass's own groups
// are everything there is to see": on a mixed-vendor node nothing is stored because no DaemonSet has
// written yet, so this pass sees its own accelerators and none of the others. A domain reduced over
// that subset can find one where the node has none, which is the overclaim nodeFeatureLabels exists
// to avoid. Nothing is removed on such a pass either, because a first pass has published nothing.
//
// The cost is that a fresh node advertises no fabric domain until a pass can read the record this
// one writes below -- up to detectEveryNMonitorRounds monitor periods. That is the safe direction:
// withholding a domain costs a co-location, inventing one costs a job.
//
// What a stored record does not establish is that every manufacturer has already written into it. It
// shows this pass whatever the other DaemonSets have published so far, and one still starting up, or
// failing detection, has published nothing -- so on a mixed-vendor node the reduction can still run
// over a subset until that one catches up. Gating on a completeness signal instead -- the
// manufacturers the node's own labels say are present -- was considered and not taken: it is a
// second source of truth, and a manufacturer those labels name whose device manager never runs would
// then withhold the domain forever on a node that does have one answer.
//
// It is a function so that reading can be checked without a cluster: the caller needs a client, and
// the difference between the error kinds is invisible in the object any of them produces.
...
func nodeFeatureLabels(
	...
	// forever.
	//
	// Only when that read succeeded. Withholding here deletes, and a read that did not happen
