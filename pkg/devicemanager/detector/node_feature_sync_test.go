...
// TestNodeWideReadSynced pins which reads may publish or withhold a node-wide label.
//
// Every error kind produces an identical object — this pass's own groups, nothing merged — so the
// distinction lives nowhere but here, and it is a decision rather than a reading: NotFound is not a
// failure, and it would be defensible to treat "nothing stored yet" as "nothing else to see". It is
// not treated that way, because on a mixed-vendor node nothing is stored precisely while the other
// manufacturer's pass has yet to run, and a domain reduced over this pass's groups alone would then
// promise the whole node a co-location its GPUs are not in.
func TestNodeWideReadSynced(t *testing.T) {
	...
	assert.True(t, nodeWideReadSynced(nil), "a stored record is the whole node")
	assert.False(t, nodeWideReadSynced(notFound),
		"the node's first pass has not seen the manufacturers that have yet to write")
	assert.False(t, nodeWideReadSynced(errors.New("etcdserver: request timed out")),
		"a read that did not happen cannot support publishing or withholding")
	assert.False(t, nodeWideReadSynced(kerrors.NewForbidden(
		schema.GroupResource{Group: "worker.gpustack.ai", Resource: "devices"}, "node-1",
		errors.New("nope"))), "nor can any other API error")
}
