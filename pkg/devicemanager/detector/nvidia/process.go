...
var _ device.AcceleratorProcessDetector = (*nvidia)(nil)

// MonitorAcceleratorProcesses implements device.AcceleratorProcessDetector: it reports which host
// processes hold each GPU and how much of it, so the device manager can attribute a share of a
...
// can observe the gap.
func (in *nvidia) MonitorAcceleratorProcesses(
	...
	defer loggerx.RecoverWithStackScanner(func(s loggerx.Scanner, e error) {
		in.logger.Error(e, "failed to monitor nvidia device processes")
		for s.Scan() {
	...
	if !noPciCheck && len(pciDevs) == 0 {
		// Something holds a carved allocation on a card no NVIDIA PCI device answers for. That is
		...
		in.logger.Info("no nvidia pci devices found for allocated accelerators",
	...
	for i := 0; i < cnt; i++ {
		...
		// timestamp: NVML keeps a few seconds of them, and the newest one per process is the
...
// acceleratorProcessesOf turns one device's two raw NVML answers into the rows the aggregator
// consumes, in NVML's own units and without flattening its semantics.
//
...
//
// NVML emits one sample per process per sampling period and only for a process that was not idle in
// that period, so one process can appear several times with different timestamps and an idle one
// does not appear at all. Summing a process's samples would count one process's activity as several
// processes'; the newest is the one that describes now.
func newestUtilizationByPID(samples []nvml.ProcessUtilizationSample) map[uint32]uint32 {
	type sample struct {
		timestamp uint64
		smUtil    uint32
	}
...
//
// A successful query holding no rows is not a refusal: NVML reports only processes that hold the
// device and only samples that were not idle, so an empty answer is a measurement of an idle device.
// NOT_FOUND — no sample at all in the window — is the same statement, and reporting it as an
// absence would hide a real zero.
func processQueryReason(ret nvml.Return) device.AcceleratorProcessReason {
	switch {
	case ret.IsSuccess(), ret == nvml.ERROR_NOT_FOUND:
		return device.AcceleratorProcessReasonNone
	case ret == nvml.ERROR_NOT_SUPPORTED, ret.IsAPIUnavailable():
	...
	case ret == nvml.ERROR_INSUFFICIENT_SIZE:
