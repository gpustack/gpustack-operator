...
//
// filepath.Join drops an empty first element, so the path collapses to /etc/hccl_rootinfo.json --
// and the container's /etc does exist, so the parent-directory guard passes, the file is absent, and
// the row reports the healthy answer on every node in the fleet. The path is therefore withheld at
// construction, which is the only place that knows a host root was never given.
func TestNewPreflighter_NoHostRootWithholdsTheRankTablePath(t *testing.T) {
	assert.Empty(t, hostRankTablePath(""),
		"no host root must not collapse onto the container's own /etc")
	assert.Equal(t, "/host"+hcclRootInfoPath, hostRankTablePath("/host"),
		"a host root that is given is joined as before")

	got := checkRankTable("")

	assert.Equal(t, device.PreflightStateUnavailable, got.State,
		"a pass that has no host root never looked, so it cannot report a healthy host")
	assert.Contains(t, got.Reason, "no host root")
