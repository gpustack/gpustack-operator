...
// because that hold is node-wide and permanent (no adoption on any accelerator, no orphan GC'd on
// any accelerator) and must not degrade the node silently. It mirrors the NVML_ERROR_IN_USE
...
// root that neither parses as a marker nor names the Pod or accelerator it belonged to. It is
...
// cardMissPrefix namespaces a per-accelerator orphan-GC miss counter in the same misses map as the
...
// in-memory instance registry. It runs single-threaded (the loop calls reconcile serially), so its
// counter maps need no lock; only the per-accelerator lock coordinates with concurrent Allocates.
type reclaimer struct {
	driver  migDriver
	podsDir string
	logger  klog.Logger
	// liveClaims returns, per accelerator UUID, the physical-slice placements live (non-terminating)
...
// accelerator's lock (never the node-wide mutex) so it never races an in-flight same-accelerator
...
// accelerator is therefore held back from the drained verdict (fail closed per accelerator, by the
// accelerator the corrupt file's name encodes), and the corrupt file itself is retired once its
// Pod — read from its path — is gone, which is what lets the accelerator converge instead of
// leaking a partition for the node's lifetime.
func (r *reclaimer) reconcile(livePodUIDs []string) {
	...
	// by the time a given accelerator is reached it may be a whole allocation old, so that check
	// re-reads the accelerator inside its own lock instead.
	...
	// An accelerator is "live" while any live Pod marks or claims it, so its marker-less orphans
	// are kept (one could be a live Pod's create-before-marker GI). markedGI indexes every GI a
	...
	// Orphan GC: a marker-less GI is destroyed only on a fully drained accelerator.
	...
	for card, orphans := range orphansByCard {
		...
		touched.Insert(key)
		// An accelerator a corrupt marker names is never treated as drained: one of these "orphans"
		// may be the instance that unreadable record owns, and destroying it would rip the partition
		// out of a running container whose only ownership record was truncated. The debounce is
		// reset, so the accelerator starts the count from scratch once its ownership is readable
		// again.
...
//     stale marker is dropped without any destroy, and a GI already gone needs only its marker
//     removed. A re-read that fails is a per-accelerator skip, never a destroy on an unvalidated
//     view.
//
...
// GPU-instance id still carries the recorded identity and placement. It reports whether every
// marker on the accelerator is now gone, and whether any destroy was rejected as in-use. A reused
// id is retained, not destroyed: only the stale marker is dropped, because the instance now at that
...
// opened with can age by a whole allocation before this accelerator is reached, and an out-of-band
...
//
// The live set is re-read ONCE for the whole accelerator rather than once per marker: several
// containers of one Pod on one accelerator are the common multi-marker case, and the enumeration is
// node-wide and expensive. Reading it once is not weaker, because the lock is held across the whole
...
// no concurrent Allocate is writing that path.
//
...
// anywhere and no orphan GC'd anywhere — for as long as it stays unreadable. Nothing here can fix
// that without destroying state it cannot account for, so the hold stands; what it must not do is
// stand silently.
//
...
// Pod). It re-scans markers under the accelerator lock and bails if the accelerator now carries ANY
// marker: create+marker is atomic under this same lock, so a marker appearing since the lock-free
// snapshot means an allocation arrived and the accelerator is no longer fully drained — its orphans
// wait for a later pass (as MetaX keeps unidentifiable orphans while any pod holds the
