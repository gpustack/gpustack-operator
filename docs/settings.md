...
| `kv-cache-backend-image` | `GPUSTACK_KV_CACHE_BACKEND_IMAGE` | `gpustack/mirrored-mooncake:0.3.13.post1-cpu` | Image every role of a [KV cache backend](kv-cache/backend.md) runs when the object does not name one itself. The default is this project's own build: it is the only image that can run [`leader.highAvailability`](kv-cache/leader.md#high-availability), because no published upstream image carries a leadership backend at all. **It does not fit every backend**, and cannot — it is a CPU build carrying TCP and EFA over DRAM, while a member's transports and the runtime it links are compiled into its wheel, so a backend on the `Ascend` transport, or a member group placed on other accelerator hardware, needs a build assembled against that runtime and names it in `spec.image`, which always wins over this. The failure mode moved with the default: blank made a missing image an admission refusal naming both places, while a default makes a wrong image a loader error at runtime. **Clearing this Setting restores the refusal** — it still allows blank for exactly that reason. Unlike the Device Manager's, this image is never derived from the operator's own: the store's leader and an inference engine's client can be builds against different accelerator generations, and a derived image would pair a leader with a runtime it cannot load. |
| `model-deployment-router-image` | `GPUSTACK_MODEL_DEPLOYMENT_ROUTER_IMAGE` | `gpustack/mirrored-llm-d-router-endpoint-picker:v0.10.0` | Endpoint picker a managed router runs when the `ModelDeployment` does not name one in `spec.router.image`. Its default is safe for every cluster at once in a way `kv-cache-backend-image`'s is not: it runs no model, so it links no accelerator runtime and cannot be paired with the wrong one. The default names a **release** rather than upstream's own tag `main`, which moves — a moving tag would let two clusters installed months apart run different pickers against the one configuration this operator renders. An image named on the object is used verbatim and is never redirected to the cluster mirror, because it is the user's own reference. |
