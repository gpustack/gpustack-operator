...
|---|---|---|
| **ResourceFlavor** | `gpustack--generic-ln-x64-4c-16g-98g--nvidia-tesla-t4-1d` — **double dash**, abbreviated `ln-x64`, CPU+device **composite**, unit-spec baked into the name | `gpustack-nvidia-tesla-t4-linux-amd64-1d` — **single dash**, full `os`/`arch`, CPU and device **split**, `count`-suffixed (`-Nc` / `-Nd`) |
...
| **InstanceType** | aggregated **virtual** API (projected from ClusterQueues; nothing stored) | a real **CRD** `instancetypes.worker.gpustack.ai` with a `.status` subresource |
| **Node feature labels** | `general.feature.gpustack.ai/generic-ln-x64` + `.z-flavor`/`.z-queue`/`.z-cohort` + per-unit `.cpu`/`.ram`/`.storage` | bare `generic` key; `.z-*` and `.cpu`/`.ram`/`.storage` **dropped**; new `.sliced.*` keys added |
...
[`cleanup-v0.5-orphans.sh`](./cleanup-v0.5-orphans.sh) removes **only** the v0.5.x orphans — objects
whose name begins with `gpustack--` (double dash) — in dependency order
(LocalQueue → ClusterQueue → Cohort → ResourceFlavor, so Kueue releases the
`kueue.x-k8s.io/resource-in-use` finalizer before the flavors are deleted). It never touches the
v0.6.x objects (single dash), your namespaces, Pods, or `Instance`s. It is idempotent and safe to
re-run; a transient API error just means you re-run it.
...
```bash
# No v0.5.x orphans remain (expect empty):
kubectl get resourceflavor,clusterqueue,cohort -A -o name | grep 'gpustack--'
...
- This procedure was validated end-to-end on a real multi-node cluster: an in-place upgrade left
  6 ResourceFlavors + 4 ClusterQueues + 4 Cohorts + a LocalQueue per namespace orphaned, and the
  script removed all of them with zero residue.
