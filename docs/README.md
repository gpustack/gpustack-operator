...
| [KV Cache Backend](kv-cache/backend.md) | How a Mooncake store is run and observed; why capacity is read rather than derived, why shrinking a group drops its cache, and why a group's identity is its position in a list | operators, contributors | ~16 min |
