...
- **`NodeCapacityReconciler`** (`node_capacity.go`) builds the capacity labels via `nodefeature.ConstructNodeCapacityLabels`, reading both the Node and the same-named `Devices` CR: the general(CPU) key presence marker plus, **for each accelerator model whose `.sliced` resource is present and > 0 in the Node capacity**, the four per-card sliced capacities that the default scheduler / kubelet count at admission time:
  ...
  `maxSlices` is the per-vendor maximum soft-slice count the DeviceManager detects into the `Devices` group's `AcceleratorsFeature` (NVIDIA 128, Ascend 63, Cambricon 16, Hygon 4, MThreads 16, MetaX 16 — each bounded by the vendor runtime's per-device user-process limit, see `pkg/devicemanager/detector`). A vendor whose runtime time-shares compute (NVIDIA / Ascend / Hygon) advertises `.sliced.cores-percentage = count × maxSlices × 100` — each slice may claim a full 100 % — while one that partitions compute spatially (Cambricon / MThreads / MetaX) caps it at `count × 100`. The four `.sliced.*` keys are **presence-gated**: the reconciler patches them only while the model's `.sliced` resource is present and positive in `Node.status.capacity`, and reverse-patches (removes) them when it disappears or reaches 0 — so a model with no slicing capability advertises no `.sliced` and gets none of the four capacities. Stale cleanup covers all four `.sliced.*` suffixes.
...
5. **GPUStack Operator DeviceManager — allocator** — at `Allocate`, performs the actual per-card device selection, container injection (visibility env / runtime-isolation preload), and records the allocation into the `Devices` ledger. Real per-slice runtime isolation exists today only for NVIDIA and Ascend (preload library + per-container compute/VRAM quota); the other four sliceable vendors (Cambricon / Hygon / MThreads / MetaX) advertise sliced capacity but their sliced `Allocate` is still a whole-card passthrough stub with no isolation — a tracked follow-up.
