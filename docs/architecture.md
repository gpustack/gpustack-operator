...
2. Labels every Node with its CPU model identity (the `cpu` label source):
   ...
   The WK later normalizes these three labels into the node's **general(CPU) node key** `${cpuManufacturer}-${cpuFamily}-${cpuID}` (e.g. `amd-25-1`, with `GenuineIntel` normalized to `intel`). When the cpu-model labels are absent or unrecognizable, the key falls back to `generic`.
...
For the **general** (CPU-only) view of every node, keyed by the general(CPU) node key `${gKey} = ${cpuManufacturer}-${cpuFamily}-${cpuID}` (or `generic`):

| Label                                                       | Value                                                                       |
|--------------------------------------------------------------|------------------------------------------------------------------------------|
| `general.${prefix}${cpuManufacturer}=true`                   | CPU manufacturer marker                                                        |
| `general.${prefix}${gKey}=true`                              | Concrete CPU model marker                                                      |
| `general.${prefix}${gKey}.family=${cpuFamily}`               | CPU family (omitted for `generic`)                                             |
...
Consider cluster `cluster-1` with 5 nodes (CPU / RAM / disk, `D` = accelerator count). node-1 and node-2 run an Intel Xeon (cpu-model `6-143` → `intel-6-143`), the rest an AMD EPYC (cpu-model `25-1` → `amd-25-1`):

| Node   | CPU model     | Accelerators | CPU | RAM   | Disk |
|--------|---------------|--------------|-----|-------|------|
| node-1 | `intel-6-143` | —            | 16C | 32G   | 100G |
| node-2 | `intel-6-143` | T4 × 1       | 4C  | 16G   | 100G |
| node-3 | `amd-25-1`    | T4 × 1       | 8C  | 32G   | 100G |
| node-4 | `amd-25-1`    | T4 × 2       | 8C  | 32G   | 100G |
| node-5 | `amd-25-1`    | A10G × 4     | 48C | 192G  | 100G |
...
|--------|------------------------------------------------------------|--------------------------------------------------------|------------------------------------------------------|
| node-1 | `gpustack--intel-6-143-16c-32g-88g`                        | `gpustack--intel-6-143-1c-2g`                          | `gpustack--intel-6-143-1c-2g`                        |
| node-2 | `gpustack--intel-6-143-4c-8g-88g`                          | `gpustack--intel-6-143-1c-2g`                          | `gpustack--intel-6-143-1c-2g`                        |
| node-2 | `gpustack--intel-6-143-4c-16g-88g--nvidia-tesla-t4-1d`     | `gpustack--intel-6-143-4c-16g--nvidia-tesla-t4-1d`     | `gpustack--intel-6-143-4c-16g--nvidia-tesla-t4-1d`   |
| node-3 | `gpustack--amd-25-1-8c-16g-88g`                            | `gpustack--amd-25-1-1c-2g`                             | `gpustack--amd-25-1-1c-2g`                           |
| node-3 | `gpustack--amd-25-1-8c-32g-88g--nvidia-tesla-t4-1d`        | `gpustack--amd-25-1-8c-32g--nvidia-tesla-t4-1d`        | `gpustack--amd-25-1-8c-32g--nvidia-tesla-t4-1d`      |
| node-4 | `gpustack--amd-25-1-8c-16g-88g` *(shared with node-3)*     | `gpustack--amd-25-1-1c-2g`                             | `gpustack--amd-25-1-1c-2g`                           |
| node-4 | `gpustack--amd-25-1-8c-32g-88g--nvidia-tesla-t4-2d`        | `gpustack--amd-25-1-4c-16g--nvidia-tesla-t4-1d`        | `gpustack--amd-25-1-4c-16g--nvidia-tesla-t4-1d`      |
| node-5 | `gpustack--amd-25-1-48c-96g-88g`                           | `gpustack--amd-25-1-1c-2g`                             | `gpustack--amd-25-1-1c-2g`                           |
| node-5 | `gpustack--amd-25-1-48c-192g-88g--nvidia-a10g-4d`          | `gpustack--amd-25-1-12c-48g--nvidia-a10g-1d`           | `gpustack--amd-25-1-12c-48g--nvidia-a10g-1d`         |
| node-5† | `gpustack--amd-25-1-48c-192g-88g--nvidia-a10g-4d-8s`      | `gpustack--amd-25-1-12c-48g--nvidia-a10g-1d-8s`        | `gpustack--amd-25-1-12c-48g--nvidia-a10g-1d`         |
...
Each ClusterQueue is then mirrored as a hash-named LocalQueue (e.g. `gpustack--amd-25-1-1c-2g` → `gpustack-fnv64-…`, 31 characters) in every non-system namespace; the `device.gpustack.ai/cluster-queue` annotation on the LocalQueue reveals which queue it fronts.

Observations worth calling out:

- **The CPU segment splits otherwise-identical device pools.** node-2 (Intel, 1×T4) and node-4 (AMD, 2×T4) both normalize to `4c-16g` per device, yet they land in **different** ClusterQueues (`gpustack--intel-6-143-…` vs `gpustack--amd-25-1-…`) — exactly the Intel+NVIDIA / AMD+NVIDIA separation. Pooling only happens between nodes with the same CPU model.
- **Per-unit normalization still pools homogeneous nodes.** Had node-2 carried the same AMD CPU as node-4, both would have joined `gpustack--amd-25-1-4c-16g--nvidia-tesla-t4-1d`, with each flavor's quota being its per-node capacity × matching node count.
- **Identical nodes share one flavor whose quota scales by node count.** node-3 and node-4 share `gpustack--amd-25-1-8c-16g-88g`; its quota in the general queue is the per-node value × 2.
- **General capacity converges per CPU model** (`gpustack--intel-6-143-1c-2g`, `gpustack--amd-25-1-1c-2g`) thanks to the default 2 Gi-per-CPU normalization, with one flavor per distinct node shape.
- **Nodes without recognizable cpu-model labels** (e.g. NFD cpu source disabled) fall back to the `generic` CPU segment and pool together.
