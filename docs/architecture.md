...
**Pod identification, cross-mode exclusion, and the one-allocation-per-Pod constraint.** The device-plugin `Allocate` call carries the assigned device IDs but not the pod identity, so the allocator maps a call to the pod being admitted by matching the oldest Pending pod on the node that requests the resource. Every node's `Allocate`s run in that node's single device-manager process, so a per-node in-process mutex serializes each workload `Allocate`'s *identify → cross-mode check → reserve* section (the durable-annotation patch runs outside it), and identification skips a pod that already holds a reservation. Together this (a) enforces the **per-card exclusive/shared/sliced cross-mode invariant** — a card kubelet assigned that another mode already holds, per the ledger `Status` or the in-process reservation, is refused with `FailedPrecondition`, so an exclusive tenant truly owns its card on every path, Kueue or raw (`ListAndWatch` itself withholds nothing — it reflects hardware health only, keeping kubelet's device accounting stable); and (b) maps a batch of identical GPU Pods admitted together (e.g. by Kueue) one-to-one to **distinct** pods, so their annotations and the per-card ledger stay correct instead of one pod being double-attributed and another lost. A consequence is that the allocator attributes **at most one workload allocation per Pod**: a Pod must request the whole-card resource (`nvidia.com/gpu[.shared|.sliced]`) in a **single** container (plus the optional `sshd` visibility sidecar). A second workload container requesting the same resource in the same Pod is *skipped* by identification (its Pod already holds a reservation), so its `Allocate` either finds no matching pod and fails, or — if an unrelated single-card Pod is pending on the node at the same instant — misattributes the card to that pod; this Pod shape is unsupported, and rejecting it at admission is a tracked follow-up. A related **known limitation**: the `sshd` visibility path must re-find its own already-reserved Pod (so it cannot use the reservation-skip), and identification alone cannot tell two SSH-enabled Instances apart when both are admitted at the same instant — a sidecar can then be granted the *other* Instance's card. Hardening the visibility path is a separate tracked follow-up.
...
- **`NodeCapacityReconciler`** (`node_capacity.go`) builds the capacity labels via `nodefeature.ConstructNodeCapacityLabels`, reading both the Node and the same-named `Devices` CR: the general(CPU) key presence marker plus, **for each accelerator model whose `.sliced` resource is present and > 0 in the Node capacity**, the four per-card sliced capacities that the default scheduler / kubelet count at admission time:
  ...
  `maxSlices` is the per-vendor maximum soft-slice count the DeviceManager detects into the `Devices` group's `AcceleratorsFeature` (NVIDIA 128, Ascend 63, Cambricon 16, Hygon 4, MThreads 16, MetaX 16 — each bounded by the vendor runtime's per-device user-process limit, see `pkg/devicemanager/detector`). A vendor whose runtime time-shares compute (NVIDIA / Ascend) advertises `.sliced.cores-percentage = count × maxSlices × 100` — each slice may claim a full 100 % — while one whose compute is **not overcommittable** (Cambricon / Hygon / MThreads / MetaX) caps it at `count × 100`. The non-overcommit form varies by vendor: Hygon is a hard spatial partition (its `vdev.conf` assigns each slice a disjoint CU bitmask, so the sum stays within one card), whereas MThreads' `cores%` is a best-effort relative weight — not a hard partition — that is nonetheless accounted as non-overcommit. The four `.sliced.*` keys are **presence-gated**: the reconciler patches them only while the model's `.sliced` resource is present and positive in `Node.status.capacity`, and reverse-patches (removes) them when it disappears or reaches 0 — so a model with no slicing capability advertises no `.sliced` and gets none of the four capacities. Stale cleanup covers all four `.sliced.*` suffixes.
...
**Five-gate admission.** Together these form a layered admission model where Kueue is a coarse gate, not the ledger — each gate produces or consumes the `.sliced.*` values at a distinct point along the path:

1. **GPUStack Operator Worker — Pod webhook** — for a sliced request it folds `.sliced.memory-percentage` / `.sliced.memory-mib` into `.sliced.units`, dividing the memory demand by the pool InstanceType's per-card `Memory` (never the user-writable LocalQueue), so a slice's memory participates in the Kueue `credits` fold-down before Kueue ever scores it.
...
4. **Default scheduler / kubelet** — node-level counting of the remaining `.sliced` / `.sliced.units` / `.sliced.cores-percentage` / `.sliced.memory-*` capacities picks the best-fitting node among that ResourceFlavor's nodes.
5. **GPUStack Operator DeviceManager — allocator** — at `Allocate`, performs the actual per-card device selection, container injection (visibility env / runtime-isolation preload), and records the allocation into the `Devices` ledger. Real per-slice runtime isolation exists today for NVIDIA and Ascend (preload library + per-container compute/VRAM quota), for MThreads (`MTHREADS_QOS_*` env vars consumed by the host sGPU kmod), and for Hygon (a per-pod `vdev.conf` — a `cores%`-derived CU bitmask + VRAM cap — mounted read-only at `/etc/vdev/docker/` and read by the host DTK/hyhal runtime); the two remaining sliceable vendors (Cambricon / MetaX) advertise sliced capacity but their sliced `Allocate` is still a whole-card passthrough stub with no isolation — a tracked follow-up. **`.sliced > 1` support is vendor-specific:** only NVIDIA meaningfully serves multiple slices co-located in one container; the other vendors emit one slice per physical card (dedup by device), so two same-card tokens are under-served while over-charged — the Pod webhook should reject `.sliced > 1` for non-NVIDIA vendors (a tracked follow-up).

The `Devices` CR `AcceleratorAllocation` ledger is the single authoritative accounting, written below the kubelet by the device-plugin `Allocate` for every allocation (Kueue-routed or not); it drives the three-view and is the store beneath gate 3, not a numbered gate itself.
