...
For the **general** (CPU-only) view of every node, keyed by the general(CPU) node key `${gKey}` — `generic` by default, or `${cpuManufacturer}-${id}` with `GPUSTACK_GENERAL_NODE_KEY_WITH_CPU_NAME=true`:
...
- **Nodes without usable CPU information** (e.g. NFD cpu source disabled) fall back to the `generic` general key even in this mode: they expose their general(CPU) view under `generic`, and their accelerated profiles pair with the `generic` CPU segment and pool together, pinning the `general.feature.gpustack.ai/generic=true` identity.
