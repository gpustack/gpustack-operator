...
- **`NodeCapacityReconciler`** (`node_capacity.go`) builds the capacity labels via `nodefeature.ConstructNodeCapacityLabels`: the general(CPU) key presence marker plus, **for each accelerator model** reported by the DM, the four per-card sliced capacities that the default scheduler / kubelet count at admission time:
