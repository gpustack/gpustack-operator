...
flowchart LR
    ...
    IT --> ITR
    DEV --> ITR
    ITR -- "owns: existence + schedule labels<br/>+ isolation; Devices-driven .status;<br/>recreate-on-delete; delete-then-wait teardown" --> CQ["ClusterQueue<br/>(isolated, no cohort)"]
    RF --> NQR
    NQR -- "owns: resource groups + StopPolicy<br/>+ AdmissionCheck ref; drain-on-delete;<br/>drain/empty when no live flavors" --> CQ
    CQ --> ITR
    CQ --> NQE
...
The chain materializes as follows (`nvidia-tesla-t4` / `nvidia-a10g` are the sanitized product keys; general capacity is the node's CPU count, the InstanceType unit spec is the fixed default — `1c/2Gi` non-accelerated, `4c/16Gi` accelerated, both `100Gi` local storage; accelerated `credits` = card count × `M`):
