...
GPUStack treats a T-Head PPU's hardware partitioning *mode* as a **manually managed** node property,
...
enables, disables, or reconfigures the mode on your behalf. Once a PPU's mode is on, though, the
...
flag for flag — so the resource-key segment is the same word, `mig`, for both manufacturers (see [Accelerator
Requests](../accelerator-requests.md#two-families-two-accelerator-populations)).

This page states the two prerequisites administrators get wrong first, then the shape of the feature and
its limits, then the enable/disable/recovery procedures. For everything the two manufacturers share — the
...
manager itself.** The manufacturer's driver refuses a mode change on a PPU that has any active process, and
because a process that merely opens the driver may hold *every* PPU's node — not only the one it is
using — a workload running on one PPU can block the mode change on a completely different PPU. Stop
...
the PPU and how many processes are still active on it, then one line per holding process, then a
failure line, while the command prints a busy error of its own. Observed on a 16-PPU host, with the
...
count as a lower bound on what you must still stop, and go back to the file descriptors. And **the PPU in
`PPU00<N>` is named by its card ordinal**, the same number `ppu-smi -i <N>` takes and the same one that names
`/dev/alixpu_ppu<N>` — not by the minor number the PPU reports, which is a different value (see
[Requesting a partition](#requesting-a-partition)).

The first line's advice to "destroy related GPU instances" applies only when instances exist; a mode change
on a PPU with no instances fails for holders alone, and **needs no device reset** once they are gone —
enabling and disabling on an idle PPU both complete in place.

**Count the operator's own control plane among the holders.** Besides the obvious ones — the accelerator
workloads and the node's device-manager Pod — the operator's worker Pod was observed opening every PPU node
...
not enumerate who is holding a PPU open. Once the kernel log has named the busy PPU(s), find the
...
`fds` count: a holder typically has one descriptor per PPU plus the shared control nodes, which is what
makes a workload on one PPU block a mode change on another.
...
PPU in front of it, and offers exactly those.

- **A profile the driver does not name is not offered.** The published resource key is the profile's
  name, and the driver seam resolves that key back to the driver's own profile id by probing every id and
  matching the driver's own name for it — a profile the driver reports without a name can never complete
  that round trip, so it is dropped (with a warning naming the PPU and the profile id) rather than
...
- **A profile whose displayed name cannot become a valid resource-name segment is excluded**, the same
  ...
  profiles](nvidia-mig.md#supported-profiles)) — the manufacturer's own display name carries a feature-name
  prefix and whitespace, stripped by the same shared normalization both the detector and the driver seam
  call, so the round trip from resource key back to driver profile stays closed.
- **A profile's memory-slice span is read from the driver's own placement records**, never divided out of
  a hardcoded per-PPU slice count — and a profile the driver enumerates *no* legal placement for is not
  ...
  allocation could never succeed. The PPU names it in a warning. This rule is shared with NVIDIA.

To see what a PPU offers, and to read the answer correctly:
...
irrespective of what is currently occupied: on a PPU carved to capacity it still lists every start, and it
...
One product's inventory, as an illustration of the shape rather than a table to rely on: a PPU reporting
...
**The published profile name carries a separator the manufacturer's own does not.** This manufacturer's display names
carry a `MIG ` prefix and a space, which normalization drops, and they spell the geometry with no separator
between its two numbers where NVIDIA writes one. So that a partition of either manufacturer reads the same way in
...
Once a PPU's mode is on, a workload asks for **one hardware instance of a named profile**, exactly as
...
(one PPU, one profile shape, one container group, `.units` webhook-derived, mutually exclusive with every
other family):
...
**One partition per allocated PPU.** As with NVIDIA, a request builds exactly one GPU Instance / Compute
Instance pair on exactly one PPU the device plugin selects itself — never a partition split across two
PPUs, and never more than one partition handed to a single container.
...
the manufacturer's two shared control nodes, the parent PPU's own device node, and the capability nodes of both
...
**Two numbers name a PPU, and only one of them names a path.** A PPU's device node and its capability
subtree are both named by its **card ordinal** — the number `ppu-smi -i <N>` takes, the `<N>` in
`/dev/alixpu_ppu<N>`, and the `ppu<N>` under the procfs capability tree. The ordinal keeps the word *card*
because it addresses a PPU rather than identifying one. The **minor number** the driver
reports for the same PPU is a *different* value: the shared `/dev/alixpu` control node occupies minor 0 of
the same character-device major as the PPUs, so a PPU's minor is its card ordinal plus one on the host this
was measured on (ordinal 14 → minor 15, uniformly across sixteen PPUs). Note this contradicts the
manufacturer documentation, which states the node-naming rule as `/dev/alixpu_ppu[minor number]`; on a real
driver the node carries the card ordinal, and building a path from the reported minor addresses **the next
PPU**. The operator uses the card ordinal for both paths and uses the reported minor only to prove the
ordinal still addresses the PPU the detector measured — it stats the node and refuses the allocation if that
node's own minor disagrees with the recorded one, rather than assuming any offset.
...
229633), and while they proved deterministic on one host, nothing in the manufacturer's contract promises that;
...
PPU, who owns the credits, or when reclaim runs is different.
...
- **One partition per allocated PPU.** A multi-partition workload asks for several Pods, the same scope
...
- **A same-profile replacement submitted the instant its predecessor is deleted can fail to start**, for
  ...
  busy-destroy retry. What that retry waits out here is narrower than the name suggests. A
  process merely holding the PPU's device node does **not** block an instance destroy — measured with the
  device manager holding all sixteen PPU nodes, a destroy succeeded on the first attempt — so the retry is
...
- **Whether the mode persists across a host reboot is not yet confirmed on real hardware.** Until your own
  ...
  A mode switch itself needs **no** device reset: on a PPU free of instances and holders, enabling and
...
2. Enable the mode. **`-mig` is a top-level device-modification flag, not a `ppu-smi mig` subcommand
   ...
   **One PPU per invocation.** `-i` does not take a PPU list for this flag: a comma-separated
   ...
   than trusting the command's own output. Several PPUs means several calls.
...
4. Verify the `Devices` capability now reports the PPU's partition profiles. Unlike NVIDIA, an unpartitioned
   T-Head PPU reports **no** slicing capability at all (T-Head has no logical/software slicing to fall
...
Run the inverse sequence: ensure no Pod is using the PPU's partitions, let them reclaim, then remove any
...
compute instances then try again` (the manufacturer's own spelling), with a non-zero exit. The operator's reclaim
...
**`-dgi` without `-gi` destroys every GPU Instance on the PPU**, not the one you were looking at. That is
convenient for clearing a PPU you own outright and a hazard everywhere else — on a node GPUStack manages,
other partitions on the same PPU may belong to running Pods.
...
and restart the node's Device Manager pod so the ledger returns the PPU to its unpartitioned capability.
...
confirmed for this manufacturer. Treat every reboot as if the mode did not survive it:
...
- It does not account for an instance you carved by hand. (It *does* delete it: an instance no allocation
  accounts for is reclaimed as an orphan once its PPU is idle — see [Limitations](#limitations).)
