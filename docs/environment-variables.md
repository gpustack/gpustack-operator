...
| `GPUSTACK_GENERAL_NODE_KEY_WITH_CPU_NAME` | `false`             | WK        | When `true`, the general(CPU) node key blends the CPU identity — the sanitized CPU name (or the NFD cpu-model family/id) plus abbreviated os/arch — e.g. `intel-xeon-platinum-8358-ln-x64`, so Kueue flavors/queues/cohorts subdivide by CPU model. When `false`, every node shares the `generic` general key.                                               |
