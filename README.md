...
**Prerequisites.** Kubernetes `>= 1.23` (required by the bundled Kueue), Helm `3.8+`, cluster-admin,
and the manufacturer driver plus container runtime on your accelerator nodes — the operator brings the
device plugin, not the driver. cert-manager is optional: the default
`global.certmanager.enabled=auto` creates its resources only when it is detected, otherwise every
component self-signs.
...
**4. Run your first sliced workload** — 20 % of one accelerator's VRAM, on a pool from step 3 whose
...
Then look inside: the container sees only its slice of a 24 GiB A10G:
...
**5. Or request an accelerator straight from a Pod.** Any Pod carrying the pool's entrance label
(`kueue.x-k8s.io/queue-name`, the **ENTRANCE** column from step 3) asks in one of four shapes.
...
One Pod asks for one family, in one container group. The admission webhook derives the `.units` keys —
never set them by hand. All seven rules, with an accepted and a rejected example each, are in
[Accelerator Requests](./docs/accelerator-requests.md).
...
[Architecture](./docs/architecture.md) for how it works · [all documentation](./docs/README.md).
...
> `enabled` switch: nothing else to deploy, one `values.yaml` for all of it. Because they belong to
> this release, `helm uninstall` also deletes Kueue's CRDs, and with them every ClusterQueue,
> ResourceFlavor and Workload in the cluster — install with `--set kueue.enabled=false` to bring your
> own instead. Add `--set cleanupOnUninstall=true` at install time to also remove the leftovers the
> worker creates at runtime. Upgrading an install from v0.7.x or earlier is a one-time ownership
> transfer — see [Migrating to the bundled subcharts](./docs/migration/to-subcharts.md).
...
2. **Device discovery** — NFD labels nodes by PCI vendor and CPU identity; the Device Manager detects
   accelerators and maintains a per-accelerator `Devices` ledger.
3. **Capacity profiling** — the worker normalizes each node's CPU/RAM/storage and per-accelerator
   capacity into labels.
4. **Queue construction & admission** — five controllers materialize those labels into a Kueue
   `ResourceFlavor` → `ClusterQueue` → `LocalQueue` chain plus an `InstanceType` CRD, gated per
   accelerator by an `AdmissionCheck`.
...
| [T-Head PPU Partitioning Operations](./docs/operation/thead-ppu-partitioning.md) | The same runbook for T-Head's own MIG-named partitioning |
