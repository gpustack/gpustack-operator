...
| #203 | a decision on whether parallelism is an API field, then per-engine formulas, then a validator | step one only, and the answer is **no field**: the degree is computed per engine from the arguments the user already wrote. The computation and the validator are not here |
...
## Decision: parallelism is NOT an API field

Two premises were re-checked on `8c6d9a80` and both held — nothing on any CR carries a parallelism
degree, and nothing validates the port window (`Ports` matches only `instance.go:181`, an
immutability comparison, against a positive control showing `model_deployment.go` reads `Template` in
eight places). So the field would have been additive rather than a rename.

FORBIDDEN anyway, and the reason is one this spec had already written down for a different option:

> under pipeline parallelism the card count is the product of two degrees, so a value derived from it
> is correct exactly where it does not matter and wrong on the deployments large enough to matter

That sentence was aimed at *deriving* the degree from the card count. **It reaches further than the
option it was written against.** A role's `extraArgs` can carry tensor, pipeline and data parallel
degrees — `-tp`, `-pp`, `-dp` and their per-engine spellings — so a single field named for the tensor
degree covers one of several, and its name does not say which. Whoever reads
`roles[].parallelism = 8` cannot tell what it excludes.

⇒ **A degree that cannot be named without enumerating the others cannot be declared; it has to be
computed.** The route is per engine, from the arguments the user already wrote, and it is recorded
under *What this spec does not deliver* rather than decided here.

**One argument survives the reversal and belongs on record**: the tensor degree is already changing
behaviour without a name. SGLang divides the configured KV segment size by it
(`global_segment_size // tp_scale_factor`, v0.5.18 `mooncake_store.py:413-416`), which this
repository records downstream at `pkg/worker/kvcache/inject/sglang.go:79-81`. That is what makes the
computation worth building — but it is an argument for **reading** the degree, not for asking the
user to restate it.
...
**Reading the degree out of `roles[].extraArgs`, per engine.** This is the route, and it is blocked
on the same missing fact from the other side: **this repository knows no engine's parallelism
argument spelling.** Measured — `git grep` over every spelling returns nothing but an unrelated
`node-dp` test constant.

Two facts bound the work when it is picked up:

- **Only the tensor degree is consumed today.** SGLang divides the configured KV segment size by it,
  `pkg/worker/kvcache/inject/sglang.go:79-81` citing `tp_scale_factor` (v0.5.18
  `mooncake_store.py:413,415-416`). Pipeline and data degrees change the card arithmetic but nothing
  this operator renders.
- **The spelling has to be read, not guessed.** The one attempt to write a concrete flag into the
  reference page during this work produced `--tensor-parallel-size`, and the only attestation
  available contradicts it (`--tensor_parallel_size` / `-tp`, underscores). A computation built on a
  guessed spelling fails open: it finds no argument and concludes the default.

REQUIRED before that computation ships: the owned-key table is paired with the renderer by an
invariant test, so the engine's parallelism key cannot be added to the table without a matching
render. That is a guard, not an obstacle.
...
- Declaring a `parallelism` field of any spelling. A role's `extraArgs` can carry tensor, pipeline
  and data degrees, so one field named for one of them covers a subset its own name does not
  delimit — and asking the user to restate a number they already wrote is what makes the two able to
  disagree.
