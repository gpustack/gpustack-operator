...
**Delivered — measured on the two-card `gfx1101` host, `2026-08-10`, against the windows the allocator
itself emitted** (generated by calling `WindowCUs` + `PackWindow` and running exactly what they
returned). Every run targets the **non-zero** physical card, named by the `GPU-<hex>` form T1 settled.

*Every packed window confines, at every offset.* `rocm-cumask-check` in mode two, one run per window:

| window | asked | occupied | rc |
| --- | --- | --- | --- |
| `0:0-11` | 6 WGP | 6 | 0 |
| `0:12-23` | 6 WGP | 6 | 0 |
| `0:0-29` | 15 WGP | 15 | 0 |
...
That closes F6's item 3 and the open question under it: a window at a non-zero, `Q = 2S`-quantised
start is honoured exactly as an offset-0 one is. The conservative choice — quantising the **start**
as well as the length — is confirmed rather than merely unfalsified, since `0:12-41` and `0:42-53`
both begin off a `0`-aligned boundary and both land to the CU.

*Disjoint packing costs nothing; sharing a window costs half.* Barrier-synchronised 8 s soak,
unmasked card = 25118 GFLOP/s, solo ceilings 5388 (`0:0-11`) and 13128 (`0:0-29`):

| scenario | per tenant | reading |
| --- | --- | --- |
| two 50 % slices, packed — `0:0-29` + `0:30-59` | 13269 / 13267 | **101 % of solo each**, 105.6 % of the whole card aggregated |
| two 25 % slices, packed — `0:0-11` + `0:12-23` | 4847 / 4853 | 90 % of solo each |
| three 50 % slices, the third falling back onto the first | 5262 / 6272 / 5329 | the two sharing `0:0-29` split it to within 1.3 %; overlap shares fairly |
| 25 + 50 + 25 — `0:0-11` + `0:12-41` + `0:42-53` | 3354 / 7493 / 3489 | ratios track 6 : 15 : 6 WGPs |

The first row is the finding this feature exists for: **a packed pair each measure their solo figure,
where an unpacked pair would have split one half between them.** The third row is its control — the
same card, the same two tenants, one window shared, and each drops to ~40 % of solo.

*A three-process cost that is not ours, recorded rather than hidden.* Any three concurrent tenants on
this card aggregate to ~57 % of it however they are masked, and the shim spec's own three-tenant row
(51.7 %) shows the same. The proportions between tenants hold; the total does not. It is a property of
the card under three HIP contexts, and no derivation changes it.

*The full injection, two containers at once.* Both containers on the same card, the two packed halves,
**different** memory quotas: each reports `rc=0` at 15 of 15 WGPs, each container's
`torch.cuda.get_device_properties(0).total_memory` reports **its own** figure (4096 and 8192 MiB), and
each `rocm-monitor` reads its own region at its own quota. Reading either region from the host as an
ordinary user is refused — the shim creates it `0600` owned by the container's uid, which is the
behaviour Risks records and, incidentally, the sharpest evidence the regions are per container.

*What this run did not cover.* The mount set as the **kubelet** applies it: the injection was applied
by hand, composed from the same constants the allocator emits, rather than by a device-plugin
registered against the node's kubelet. And CDNA — no run of this feature's Go composed an injection
on a multi-XCC or a `gfx90a` card. Both are carried in Open Questions rather than implied to be done.
...
- [x] **T8 · Verification on the RDNA host**
      Blocked by: T5, T7
...
- ~~**Must an RDNA window's start be shader-engine-aligned, or only its length?**~~ **Answered by T8,
  in the safe direction.** Six packed windows, four of them at non-zero `Q = 2S`-quantised starts,
  were each honoured to the exact WGP. What is still unmeasured is whether a *finer* start would also
  work — the quantised one is confirmed, the relaxation is not, and nothing needs it.
