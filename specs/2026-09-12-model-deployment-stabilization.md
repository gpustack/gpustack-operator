...
Measured on `origin/main` at `f134b54a`.
...
| `status.roles[].kind` | Plain string, no enum | The spec field carries the enum; the status field was left until the writer was known to stay inside it |
...
| --- | --- | --- |
| S6 — single-role `ModelDeployment` | Building, blocked on hardware | Only its shipped code surface is used; nothing here waits on its measurements |
...
- [ ] **T2 · The status kind enum, proved from the writer**
      Blocked by: None
      ...
      Gate: review
      Acceptance: the enum marker on the status field, not on the type. A test enumerating every
      value the producing function can return and asserting each is in the enum — sourced from the
      writer, not from the enum. The generated CRD carries the enum, and a status write of each
      producible value is accepted against it.
      Verify: run `make generate` from the module-suffixed checkout, sync back, then `make generate
      && git diff --exit-code` there; `go build ./api/...`
