...
| 60 | The connector name we render is one that engine's own factory can resolve, for both vLLM-family engines. NOT YET EXERCISED on any cluster | `pkg/worker/kvcache/inject/vllm.go` (`vllmConnectorFor`), `pkg/worker/kvcache/inject/engine.go` | yes (confirm) | as CASE 53, plus `E2E_VLLM_IMAGE` / `E2E_VLLM_ASCEND_IMAGE`; each engine row self-skips without its image |
