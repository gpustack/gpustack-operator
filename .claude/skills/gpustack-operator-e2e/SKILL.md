...
**Layout:**
- `../_e2e-lib/scripts/` — `preflight.sh`, `build-load.sh <TAG>`, `deploy.sh <NS> <TAG>`, `assert-core.sh <NS>`, `teardown.sh <NS>` (self-contained cleanup; mirrors the chart's `cleanup.sh`), plus the cluster-lifecycle trio `cluster-auth.sh <modality>` (read-only), `provision.sh <modality>` and `destroy.sh <modality>`.
...
| 11 | Logical-slice per-card accounting: SL view + per-card OnceMax (spread observed best-effort) | `pkg/deviceplugin/server.go`, `pkg/worker/controllers/worker/{node_devices_admission,instance_type}.go` | `cases/case-11.sh` | yes (confirm) | real GPU · skips: <2 logically sliceable cards |
...
| 28 | What the SSH sidecar of a partition-backed workload actually sees (observation) | `pkg/deviceplugin/server.go`, `pkg/devicemanager/allocator/nvidia/**`, `pkg/worker/controllers/worker/instance.go`, `pack/ssh-server/**` | `cases/case-28.sh` | yes (confirm) | partition-capable NVIDIA card **+ node SSH** · skips: not partition-capable / no partition profile / no `MIG_NODE_SSH` (exit 2) |
...
- **CASES 25, 31, 32 and 34 record observations rather than assert thresholds** — the ledger-vs-hardware
  ...
  double-books a card (32). CASE 28 is observational by default and becomes a guard with `F11_EXPECT=own`.
