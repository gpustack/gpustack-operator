...
| 23 | NVIDIA MIG dynamic-allocation lifecycle (logical→enable→carve→exclusion→reuse→reclaim→disable) | `pkg/devicemanager/allocator/nvidia/**`, `pkg/devicemanager/detector/nvidia/device.go`, `binding/nvml/**`, `pkg/deviceplugin/**`, `pkg/worker/controllers/worker/node_capacity.go`, `pkg/worker/webhooks/worker/pod.go` | `cases/case-23.sh` | yes (confirm) | MIG-capable NVIDIA card **+ node SSH** · skips: not MIG-capable / no `MIG_NODE_SSH` |
