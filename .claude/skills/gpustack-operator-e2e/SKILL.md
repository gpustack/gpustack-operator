...
| 14 | Multiple slices coexist on one physical card within budget | `pkg/worker/controllers/worker/{node_devices_admission,instance_type}.go`, `pkg/deviceplugin/**` | yes (confirm) | real GPU, logical slicing |
...
| 37 | Instance metrics subresource serves the current instance-scoped utilization | `pkg/worker/extensionapis/worker/**`, `pkg/kubemetrics/**`, `pkg/devicemanager/**`, `pkg/utils/datax/snapshot.go`, `api/worker/v1/instance.metrics.go` | yes (confirm) | any (chain materialized) |
