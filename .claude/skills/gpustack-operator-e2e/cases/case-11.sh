#!/usr/bin/env bash
#
# CASE 11 — Logical slices spread across cards; per-card VRAM is not over-committed   (MUTATING, self-recovering; AUTO-SKIPS without >=2 logically sliceable cards)
#
#   case-11.sh <NS>
#
# Goal:        ASSERTS the deterministic part of per-card logical-slice accounting — the per-card ledger
#              records REAL units, so the SL view's remaining drops below capacity (it used to
#              stay ~full regardless of slice size), and the SL OnceMaxRequest is per-card (<=100),
...
#              AUTO-SKIPS (exit 0) otherwise. Picks the InstanceType whose pool the target node belongs to.
# Inputs:      All real, nothing mocked — up to 4 sequential 60% (memory + cores) logical slices pinned to the
...
if [ -z "$SLICE_NODE" ] || [ "${NCARDS:-0}" -lt 2 ]; then
  ...
  echo "needs real multi-card accelerator hardware, with the cards NOT in a hardware partitioning mode, to prove"
  echo "logical slices spread. Run it on such a node."
  exit 0
fi
# Bound the test to at most 4 slices to keep it quick.
N="$NCARDS"; [ "$N" -gt 4 ] && N=4
echo "[case-11] spread target: ${SLICE_NODE} with ${NCARDS} logically sliceable card(s); submitting ${N} × ${PCT}% slices"
...
# 2. The SL view's remaining reflects the committed slices (not ~full) — per-card units ledger.
...
if [ -n "$srem" ] && [ -n "$scap" ] && [ "$srem" -lt "$scap" ]; then
  record PASS "SL view reflects occupancy" "acceleratorSliced remaining ${srem} < capacity ${scap}"
else
  record FAIL "SL view reflects occupancy" "remaining='${srem:-?}' not < capacity='${scap:-?}' (ledger not units-accurate)"
fi

# 4. The SL OnceMaxRequest is per-card (<=100), not a node card-sum.
sorm=$(kubectl get instancetypes.worker.gpustack.ai "$IT" -o jsonpath='{.status.acceleratorSliced.onceMaxRequest}' 2>/dev/null)
{ [ -n "$sorm" ] && [ "$sorm" -le 100 ]; } \
  && record PASS "SL OnceMaxRequest is per-card (<=100)" "acceleratorSliced onceMaxRequest ${sorm}" \
  || record FAIL "SL OnceMaxRequest is per-card (<=100)" "onceMaxRequest='${sorm:-?}' > 100 (node card-sum, not per-card)"

echo
echo "== CASE 11 — Logical slices spread across cards; per-card VRAM is not over-committed =="
{
...
if [ "$FAILS" -ne 0 ]; then
  echo
  echo "FAILED ${FAILS} check(s). The per-card ledger must record real .sliced.units so the SL view's"
  echo "remaining drops below capacity, and the SL OnceMaxRequest must be per-card (<=100). (Spread"
