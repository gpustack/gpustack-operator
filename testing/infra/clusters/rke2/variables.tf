...
variable "registry_mirrors" {
  ...
  # system_default_registry moves only the images RKE2 ships with; everything a chart pulls still
  ...
  description = "Per-registry mirror endpoints written to each node's registries.yaml, e.g. {\"docker.io\" = [\"https://mirror.example.com\"]}. Unset follows the mirror ('cn' derives community docker.io proxies); {} writes no file."
