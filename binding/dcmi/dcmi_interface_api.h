...
#define TOPO_INFO_MAX_LENGTH 32
...
typedef struct dcmi_chip_info_v2 {
    unsigned char chip_type[MAX_CHIP_NAME_LEN];
    unsigned char chip_name[MAX_CHIP_NAME_LEN];
    unsigned char chip_ver[MAX_CHIP_NAME_LEN];
    unsigned int aicore_cnt;
...
typedef struct dcmi_pcie_info_all {
...
typedef struct dcmi_die_id {
    unsigned int soc_die[DIE_ID_COUNT];
} dcmi_die_id;

typedef struct dcmi_ecc_info {
    int enable_flag;
...
typedef struct dcmi_memory_info {
    unsigned long long memory_size;        /* unit:MB */
    unsigned int freq;
    unsigned int utiliza;
} dcmi_memory_info;

typedef struct dcmi_get_memory_info_stru {
    unsigned long long memory_size;        /* unit:MB */
...
enum {
    ...
    DCMI_TOPO_TYPE_HCCS_SW,      /* 通过1520交换的HCCS链接 */
...
enum dcmi_vdev_mng_sub_cmd {
    ...
    DCMI_VMNG_SUB_CMD_GET_VDEV_ACTIVITY,
...
typedef struct dcmi_base_resource {
...
typedef struct dcmi_computing_resource {
    /* accelator resource */
    float aic;
    float aiv;
    unsigned short dsa;
...
} dcmi_computing_resource;

typedef struct dcmi_media_resource {
    /* dvpp resource */
    float jpegd;
    float jpege;
    float vpc;
...
typedef struct dcmi_create_vdev_out {
    ...
    unsigned int vfg_id;
    unsigned char reserved[DCMI_VDEV_FOR_RESERVE];
} dcmi_create_vdev_out;

typedef struct dcmi_create_vdev_res_stru {
    unsigned int vdev_id;
    unsigned int vfg_id;
    char template_name[TEMPLATE_NAME_LEN];
    unsigned char reserved[64];
} dcmi_create_vdev_res_stru;

typedef struct dcmi_vdev_query_info {
...
} dcmi_vdev_query_info;

/* for single search */
typedef struct dcmi_vdev_query_stru {
    unsigned int vdev_id;
    struct dcmi_vdev_query_info query_info;
} dcmi_vdev_query_stru;
...
typedef struct dcmi_ub_ping_mesh_operate {
    ...
    int task_interval;
    int task_id;
} dcmi_ub_ping_mesh_operate;

typedef struct dcmi_ub_ping_mesh_info {
    char src_eid[UB_ADDR_MAX_LEN];
    char dst_eid_list[UB_DST_EIT_MAX_LEN][UB_ADDR_MAX_LEN];
...
} dcmi_ub_ping_mesh_info;

// ub ping mesh for A5 -- end

typedef struct dcmi_soc_free_resource {
    unsigned int vfg_num;
    unsigned int vfg_bitmap;
    struct dcmi_base_resource base;
    struct dcmi_computing_resource computing;
    struct dcmi_media_resource media;
} dcmi_soc_free_resource;

typedef struct dcmi_soc_total_resource {
    ...
    struct dcmi_computing_resource computing;
    struct dcmi_media_resource media;
} dcmi_soc_total_resource;

typedef struct dcmi_spod_info {
    unsigned int sdid;
...
typedef struct dcmi_dms_fault_event {
...
} dcmi_dms_fault_event;

typedef struct dcmi_event {
...
typedef struct dcmi_event_filter {
...
typedef struct dcmi_proc_mem_info {
    int proc_id;
    // unit is byte
    unsigned long proc_mem_usage;
} dcmi_proc_mem_info;

typedef struct dcmi_board_info {
...
typedef struct dcmi_pcie_link_bandwidth_info {
    int profiling_time;
...
typedef struct dcmi_hccs_statistic_info {
...
} dcmi_hccs_statistic_info;

typedef struct dcmi_hccs_statistic_info_u64 {
...
} dcmi_hccs_statistic_info_u64;

typedef struct dcmi_hccs_bandwidth_info {
    int profiling_time;
    double total_txbw;
    double total_rxbw;
    double tx_bandwidth[HCCS_MAX_PCS_NUM];
    double rx_bandwidth[HCCS_MAX_PCS_NUM];
} dcmi_hccs_bandwidth_info;

typedef struct dcmi_sio_crc_err_statistic_info {
    unsigned short tx_error_count;
    unsigned short rx_error_count;
    unsigned char reserved[8];
} dcmi_sio_crc_err_statistic_info;

typedef struct dcmi_elabel_info {
...
typedef struct dcmi_hccsping_mesh_operate {
    ...
    int task_interval;
    int task_id;
} dcmi_hccsping_mesh_operate;

typedef struct dcmi_hccsping_mesh_info {
    ...
    int dest_num;
} dcmi_hccsping_mesh_info;

typedef struct dcmi_multi_utilization_info {
...
typedef struct dcmi_ub_port_info {
    int udie_id;
    int port_id;
} dcmi_ub_port_info;

typedef struct dcmi_port_pkt_stats_info {
    int port_mode;
...
typedef struct dcmi_memory_info_stru {
    unsigned long long memory_size;
    unsigned int freq;
    unsigned int utiliza;
} dcmi_memory_info_stru;

typedef struct dcmi_soc_die_stru {
    unsigned int soc_die[5];
} dcmi_soc_die_stru;

typedef struct dsmi_soc_die_stru {
    unsigned int soc_die[5];
} dsmi_soc_die_stru;
