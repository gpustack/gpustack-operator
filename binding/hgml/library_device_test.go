...
// size, so the version word a caller writes is the only thing telling them apart and a library
...
func TestGpuInstanceProfileInfoVersionsShareASize(t *testing.T) {
	...
	assert.Equal(t, unsafe.Sizeof(v2), unsafe.Sizeof(v3),
		"v2 and v3 are the same size, which is what lets a library confuse them")
}

// profileName renders a profile name into the C char array the library reports it in.
...
// asV3Buffer returns what a v3-typed buffer holds after a library has filled it with the v2 layout:
...
func TestDescribesSameProfile(t *testing.T) {
	...
	cases := []struct {
		...
		{
			// The case the code exists for: the library reported success and filled the buffer with
		...
		{
			// The name is what is compared, not the array holding it, so whatever a library leaves
